...
Set `AI_WEB_SEARCH_ENABLED=true` to expose the provider-neutral `search_public_web` tool. The initial adapter performs a separate OpenAI Responses API request with live web search and returns normalized URL citations. Atlas sends only the screened, generic public query through that boundary—not the conversation history, retrieved firm records, workspace context, or canonical source objects. Searches containing known firm or matter identifiers, case numbers, email addresses, or phone numbers are rejected. When a model accidentally includes private context, Atlas returns a safe tool correction so the model can separate the private Atlas lookup from a sanitized public search instead of ending the conversation. Temporary web-provider failures likewise become an honest conversational limitation rather than a broken chat turn. `AI_WEB_SEARCH_CONTEXT_SIZE` accepts `low`, `medium`, or `high` and defaults to `medium`.
...
Microsoft 365 uses one delegated OAuth connection for Outlook inbox synchronization, primary-calendar synchronization, attorney-approved calendar additions, and attorney-approved confirmation email. Atlas requests `Mail.Read`, `Mail.Send`, `Calendars.ReadWrite`, and `offline_access`. `Mail.Send` is invoked only after an authenticated attorney approves the corresponding action; Atlas does not autonomously send a proposed message. Atlas retains Microsoft Graph delta links for incremental changes and source deletions, normalizes event times to UTC, and displays synchronized events in the canonical Calendar workspace. Native intelligence can propose a source-supported court date, scheduled consultation, deposition, deadline, or meeting from firm events; the proposal appears in **While You Were Gone**, and no Atlas or Outlook calendar event is created until an attorney approves it. Approved events are added to that user's connected Microsoft calendar without automatically inviting attendees, and pending writes retry safely. Configure `MICROSOFT_365_CLIENT_ID`, `MICROSOFT_365_CLIENT_SECRET`, `MICROSOFT_365_TENANT`, and the public HTTPS origin in `PUBLIC_BASE_URL`; Atlas fixes the callback to that server origin instead of accepting a browser-selected redirect. Existing connections must be disconnected and reconnected after adding `Mail.Send` to the Entra registration. See `docs/MICROSOFT_365_SETUP.md` for the Entra registration, consent, deployment, and mandatory live-tenant acceptance procedure.

Public website consultation requests use `ATLAS_PUBLIC_WEBSITE_CONNECTIONS`, a JSON array of `{websiteId, workspaceId, targetUserId, token, timeZone}` records. Atlas stores each request as a firm-scoped canonical object, identifies open times from the connected attorney calendar, and places the request in that attorney's **While You Were Gone** feed. When the firm enables **Text me when a website consultation needs a time approved** in Communications and saves the attorney mobile number, Atlas sends the options to the attorney by text and attempts the same operational notice through the attorney's connected mailbox. These are proposals only: selecting a time sends it to the consented prospective client by text and email without creating a calendar event. A client `YES` reply triggers a transaction-locked availability recheck, creates and publishes the event, and sends the client and attorney final text/email details. One client counteroffer and one final attorney proposal are supported; denial of the final proposal opens the ordinary Atlas text thread for direct scheduling. If a provider is unavailable or the time becomes occupied, Atlas records the pending/conflict state instead of claiming completion or double-booking.
