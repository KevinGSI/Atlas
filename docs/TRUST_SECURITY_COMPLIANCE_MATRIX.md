...
| Login visibility | Firm administrators can inspect login and activity history | Implemented security-event and firm-session APIs and Settings interface | Add retention policy, alert rules, export, anomaly detection, and verified proxy-aware client IP collection |
...
| Malware protection | Daily malware scans publicly described | Implemented technical boundary: uploads and connected-mail files require signature verification and a clean scanner verdict before durable storage or intelligence; production requires the replaceable ClamAV adapter and fails closed on scanner errors | Operate private scanners with monitored engine/signature updates, add content disarm where appropriate, alerting, retained evidence, periodic rescanning, and independent control testing |
