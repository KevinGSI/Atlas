...
export class OpenAiResponsesProvider {
  ...
  async embedTexts(input){
    ...
    return {vectors:ordered.map(item=>item.embedding),provider:'openai',model:body.model??this.embeddingModel,indexModel:this.embeddingModel,dimensions:this.embeddingDimensions,usage:{inputTokens:body.usage?.prompt_tokens??0,outputTokens:0,totalTokens:body.usage?.total_tokens??0}};
  }
