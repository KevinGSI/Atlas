...
export class DigitalTwinImpactConsumer {
  ...
  async handle(event){return this.repository.transaction(async(repository)=>{const relationships=await repository.listRelationships(event.workspaceId);const affected=new Set(event.affectedObjectIds);const impacted=new Set();for(const relationship of relationships){if(affected.has(relationship.fromObjectId))impacted.add(relationship.toObjectId);if(affected.has(relationship.toObjectId))impacted.add(relationship.fromObjectId);}let queued=0;for(const objectId of impacted){if(affected.has(objectId))continue;const marker=`canonical-impact:${event.id}:${objectId}`;if(!await repository.createAutomationMarker(event.workspaceId,marker,this.clock()))continue;const object=await repository.getObject(event.workspaceId,objectId);const now=this.clock();await repository.createIntelligenceJob({id:createId('inj'),workspaceId:event.workspaceId,triggerType:'graph.impact',objectId,eventId:event.id,status:'pending',attempts:0,payload:{causeEventId:event.id,correlationId:event.correlationId,sourceObjectIds:event.affectedObjectIds,object},result:null,provider:null,errorCode:null,availableAt:now,lockedAt:null,createdAt:now,completedAt:null});queued+=1;}return {queued};});}
}
