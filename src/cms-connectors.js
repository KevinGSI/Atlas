...
export async function runCmsSyncScheduler(service,options={}){const signal=options.signal;const intervalMs=options.intervalMs??300000;const onCycle=options.onCycle??(()=>{});while(!signal?.aborted){const cycle=options.leaseCoordinator?await options.leaseCoordinator.runExclusive(options.leaseName??'cms-sync',()=>service.syncAll()):{acquired:true,result:await service.syncAll()};if(cycle.acquired)onCycle(cycle.result);if(signal?.aborted)break;await new Promise((resolve)=>{const timer=setTimeout(resolve,intervalMs);signal?.addEventListener('abort',()=>{clearTimeout(timer);resolve();},{once:true});});}}
