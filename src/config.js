...
function publicWebsiteConnectionsConfig(env){if(!env.ATLAS_PUBLIC_WEBSITE_CONNECTIONS)return[];let values;try{values=JSON.parse(env.ATLAS_PUBLIC_WEBSITE_CONNECTIONS);}catch{throw new Error('ATLAS_PUBLIC_WEBSITE_CONNECTIONS must be a JSON array');}if(!Array.isArray(values)||values.length>25)throw new Error('ATLAS_PUBLIC_WEBSITE_CONNECTIONS must contain no more than 25 website definitions');const seen=new Set();return values.map((item,index)=>{if(!item||typeof item!=='object'||Array.isArray(item))throw new Error(`ATLAS_PUBLIC_WEBSITE_CONNECTIONS[${index}] must be an object`);const websiteId=String(item.websiteId??'').trim();const workspaceId=String(item.workspaceId??'').trim();const targetUserId=String(item.targetUserId??'').trim();const token=String(item.token??'');const timeZone=String(item.timeZone??'America/New_York').trim();if(!/^[a-z0-9][a-z0-9_-]{1,79}$/i.test(websiteId)||!workspaceId||!targetUserId||token.length<32)throw new Error(`ATLAS_PUBLIC_WEBSITE_CONNECTIONS[${index}] requires a valid websiteId, workspaceId, targetUserId, and 32-character token`);if(seen.has(websiteId))throw new Error(`ATLAS_PUBLIC_WEBSITE_CONNECTIONS contains duplicate websiteId ${websiteId}`);seen.add(websiteId);try{new Intl.DateTimeFormat('en-US',{timeZone});}catch{throw new Error(`ATLAS_PUBLIC_WEBSITE_CONNECTIONS[${index}].timeZone is invalid`);}return{websiteId,workspaceId,targetUserId,token,timeZone};});}

function divorceDigitalCatalogConfig(env){if(!env.ATLAS_DIVORCE_DIGITAL_SERVICE_CATALOG)return[];let values;try{values=JSON.parse(env.ATLAS_DIVORCE_DIGITAL_SERVICE_CATALOG);}catch{throw new Error('ATLAS_DIVORCE_DIGITAL_SERVICE_CATALOG must be a JSON array');}if(!Array.isArray(values)||values.length>50)throw new Error('ATLAS_DIVORCE_DIGITAL_SERVICE_CATALOG must contain no more than 50 service definitions');return values;}
...
export function loadConfig(env = process.env) {
  ...
  const publicWebsiteConnections=publicWebsiteConnectionsConfig(env);
  const divorceDigitalServiceCatalog=divorceDigitalCatalogConfig(env);
  ...
  return {
    ...
    publicWebsiteConnections,
    divorceDigitalServiceCatalog,
