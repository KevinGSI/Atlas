...
function route(method, pathname) {
  const patterns = [
    ...
    ['GET', /^\/v1\/workspaces\/([^/]+)\/firm-communications$/, 'firmCommunications'],
    ['POST', /^\/v1\/workspaces\/([^/]+)\/firm-communications\/call$/, 'prepareFirmCall'],
    ['POST', /^\/v1\/workspaces\/([^/]+)\/firm-communications\/text-drafts$/, 'createFirmTextDraft'],
    ['POST', /^\/v1\/workspaces\/([^/]+)\/firm-communications\/email-drafts$/, 'createFirmEmailDraft'],
    ['POST', /^\/v1\/workspaces\/([^/]+)\/firm-communications\/email-drafts\/([^/]+)\/send$/, 'sendFirmEmailDraft'],
    ['POST', /^\/v1\/workspaces\/([^/]+)\/firm-communications\/([^/]+)\/link-case\/([^/]+)$/, 'linkFirmCommunicationToMatter'],
    ...
    ['POST', /^\/v1\/workspaces\/([^/]+)\/matters\/([^/]+)\/client-communications\/email-drafts\/([^/]+)\/send$/, 'sendMatterClientEmailDraft'],
  ...
  return async (request, response) => {
    ...
    try {
      ...
      switch (match.name) {
        ...
        case 'firmCommunications': {if(!caseCommunications)throw new AtlasError('CASE_COMMUNICATIONS_NOT_CONFIGURED','Firm communications are unavailable',503);result=await caseCommunications.firmDirectory(workspaceId);break;}
        case 'prepareFirmCall': {if(!caseCommunications)throw new AtlasError('CASE_COMMUNICATIONS_NOT_CONFIGURED','Firm communications are unavailable',503);result=await caseCommunications.prepareFirmCall(workspaceId,await readJson(request,config.maxBodyBytes),user?.id??'system');break;}
        case 'createFirmTextDraft': {if(!caseCommunications)throw new AtlasError('CASE_COMMUNICATIONS_NOT_CONFIGURED','Firm communications are unavailable',503);result=await caseCommunications.createFirmTextDraft(workspaceId,await readJson(request,config.maxBodyBytes),user?.id??'system');break;}
        case 'createFirmEmailDraft': {if(!caseCommunications)throw new AtlasError('CASE_COMMUNICATIONS_NOT_CONFIGURED','Firm communications are unavailable',503);result=await caseCommunications.createFirmEmailDraft(workspaceId,await readJson(request,config.maxBodyBytes),user?.id??'system');break;}
        case 'sendFirmEmailDraft': {if(!caseCommunications)throw new AtlasError('CASE_COMMUNICATIONS_NOT_CONFIGURED','Firm communications are unavailable',503);result=await caseCommunications.sendFirmEmailDraft(workspaceId,objectId,await readJson(request,config.maxBodyBytes),user?.id??'system');break;}
        case 'linkFirmCommunicationToMatter': {if(!caseCommunications)throw new AtlasError('CASE_COMMUNICATIONS_NOT_CONFIGURED','Firm communications are unavailable',503);result=await caseCommunications.linkFirmCommunicationToMatter(workspaceId,objectId,match.params[2],user?.id??'system');break;}
        ...
        case 'sendMatterClientEmailDraft': {if(!caseCommunications)throw new AtlasError('CASE_COMMUNICATIONS_NOT_CONFIGURED','Case client communications are unavailable',503);result=await caseCommunications.sendEmailDraft(workspaceId,objectId,match.params[2],await readJson(request,config.maxBodyBytes),user?.id??'system');break;}
