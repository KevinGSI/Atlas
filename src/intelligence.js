  ...
  async processNext() {
    ...
    try {
      ...
      return await this.repository.transaction(async (repository) => {
        ...
        const persistedResult={...result};delete persistedResult.knowledgeEmbeddings;return repository.completeIntelligenceJob(job.id, persistedResult, selected.name, this.clock());
      });
    } catch (error) {
