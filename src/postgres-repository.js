...
export class PostgresRepository {
  ...
  async createCmsRecordLink(v){const r=await this.executor.query('INSERT INTO atlas_cms_record_link (id,workspace_id,connection_id,external_type,external_id,atlas_object_id,source_updated_at,source_checksum,last_synced_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *',[v.id,v.workspaceId,v.connectionId,v.externalType,v.externalId,v.atlasObjectId,v.sourceUpdatedAt,v.sourceChecksum,v.lastSyncedAt]);return cmsRecordLink(r.rows[0]);}
  async updateCmsRecordLink(id,changes){const r=await this.executor.query('UPDATE atlas_cms_record_link SET source_updated_at=$2,source_checksum=$3,last_synced_at=$4 WHERE id=$1 RETURNING *',[id,changes.sourceUpdatedAt,changes.sourceChecksum,changes.lastSyncedAt]);if(!r.rows[0])throw new AtlasError('CMS_RECORD_LINK_NOT_FOUND','CMS record link not found',404);return cmsRecordLink(r.rows[0]);}
