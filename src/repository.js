...
export class InMemoryRepository {
  ...
  #documentKnowledgeEmbeddings = new Map();
  #documentKnowledgeChunks = new Map();
  ...
  async transaction(work) {
    ...
    try { const result=await work(this);const mutated=new Set();for(const [id,value] of this.#objects)if(beforeObjects.get(id)!==JSON.stringify(value))mutated.add(id);for(const [id,relationship] of this.#relationships)if(!beforeRelationships.has(id)){mutated.add(relationship.fromObjectId);mutated.add(relationship.toObjectId);}const covered=new Set([...this.#canonicalEvents].filter(([id])=>!beforeEvents.has(id)).flatMap(([,event])=>event.affectedObjectIds));const missing=[...mutated].filter((id)=>!covered.has(id));if(missing.length)throw new AtlasError('CANONICAL_EVENT_REQUIRED','Material canonical mutations require event coverage',500,{objectIds:missing});return result; }
      ...
      [this.#workspaces, this.#objects, this.#relationships, this.#events, this.#users, this.#memberships, this.#workspaceInvitations, this.#subscriptions, this.#audits, this.#refreshSessions, this.#passwordResets, this.#loginThrottles, this.#aiRuns, this.#aiConversations, this.#aiMessages, this.#aiActionProposals, this.#intelligenceJobs, this.#intelligenceObservations,this.#documentKnowledgeEmbeddings,this.#documentKnowledgeChunks, this.#ingestionRecords,this.#cmsAuthorizations,this.#cmsConnections,this.#cmsRecordLinks,this.#encryptedSecrets,this.#awarenessItems,this.#awarenessReceipts] = snapshot;
  ...
  createDocumentKnowledgeEmbedding(value){const key=`${value.observationId}:${value.provider}:${value.model}`;if(!this.#documentKnowledgeEmbeddings.has(key))this.#documentKnowledgeEmbeddings.set(key,clone(value));return clone(this.#documentKnowledgeEmbeddings.get(key));}
  listDocumentKnowledgeEmbeddings(workspaceId,model){return [...this.#documentKnowledgeEmbeddings.values()].filter(value=>value.workspaceId===workspaceId&&(!model||value.model===model)).map(clone);}
  listUnembeddedDocumentObservations(workspaceId,model,limit){const indexed=new Set([...this.#documentKnowledgeEmbeddings.values()].filter(value=>value.workspaceId===workspaceId&&value.model===model).map(value=>value.observationId));return [...this.#intelligenceObservations.values()].filter(value=>value.workspaceId===workspaceId&&value.status!=='rejected'&&!indexed.has(value.id)&&this.#objects.get(value.sourceObjectId)?.dimension==='document').sort((a,b)=>a.createdAt.localeCompare(b.createdAt)).slice(0,limit).map(value=>({...clone(value),documentTitle:this.#objects.get(value.sourceObjectId).title}));}
  createDocumentKnowledgeChunk(value){const key=`${value.workspaceId}:${value.sourceObjectId}:${value.model}:${value.ordinal}`;if(!this.#documentKnowledgeChunks.has(key))this.#documentKnowledgeChunks.set(key,clone(value));return clone(this.#documentKnowledgeChunks.get(key));}
  listDocumentKnowledgeChunks(workspaceId,model){return [...this.#documentKnowledgeChunks.values()].filter(value=>value.workspaceId===workspaceId&&(!model||value.model===model)).map(clone);}
  listUnchunkedStoredDocuments(workspaceId,model,limit){const indexed=new Set([...this.#documentKnowledgeChunks.values()].filter(value=>value.workspaceId===workspaceId&&value.model===model).map(value=>value.sourceObjectId));return [...this.#objects.values()].filter(value=>value.workspaceId===workspaceId&&!value.deletedAt&&value.dimension==='document'&&String(value.state?.storageRef??'').startsWith(`atlas-blob://${workspaceId}/`)&&!indexed.has(value.id)).sort((a,b)=>a.createdAt.localeCompare(b.createdAt)).slice(0,limit).map(clone);}
