...
export class AtlasService {
  ...
  async searchTwin(workspaceId, query) {
    const text=required(query,'query').trim().toLowerCase();if(text.length<2||text.length>1000)throw new AtlasError('VALIDATION_ERROR','firm twin query must contain 2 to 1000 characters',400);
    const terms=[...new Set(text.split(/[^a-z0-9]+/).filter(term=>term.length>2))];const relevance=value=>{const candidate=String(value??'').toLowerCase();return (candidate.includes(text)?100:0)+terms.reduce((score,term)=>score+(candidate.includes(term)?1:0),0);};
    const [objects,relationships,events,observations,actions]=await Promise.all([this.repository.listObjects(workspaceId,{}),this.repository.listRelationships(workspaceId),this.repository.listEvents(workspaceId),this.repository.listIntelligenceObservations(workspaceId,'accepted'),this.repository.listAiActionProposals(workspaceId)]);
    const rankedObjects=objects.map(item=>({item,relevance:relevance(`${item.title} ${item.type} ${item.dimension} ${JSON.stringify(item.state)}`)})).filter(entry=>entry.relevance).sort((a,b)=>b.relevance-a.relevance||String(b.item.updatedAt).localeCompare(String(a.item.updatedAt))).slice(0,25);const objectIds=new Set(rankedObjects.map(entry=>entry.item.id));
    const rankedObservations=observations.map(item=>({item,relevance:relevance(`${item.kind} ${JSON.stringify(item.data)}`)})).filter(entry=>entry.relevance).sort((a,b)=>b.relevance-a.relevance).slice(0,25);rankedObservations.forEach(entry=>objectIds.add(entry.item.sourceObjectId));
    const relatedRelationships=relationships.filter(item=>objectIds.has(item.fromObjectId)||objectIds.has(item.toObjectId)).slice(0,50);const relatedEvents=events.filter(item=>objectIds.has(item.parentObjectId)||(item.relatedObjectIds??[]).some(id=>objectIds.has(id))).slice(-50);const relatedActions=actions.filter(item=>objectIds.has(item.resultObjectId)||relevance(`${item.actionType} ${JSON.stringify(item.input)}`)).slice(0,25);
    return {scope:'firm',query:text,objects:rankedObjects.map(entry=>({...entry.item,relevance:entry.relevance})),relationships:relatedRelationships,events:relatedEvents,observations:rankedObservations.map(entry=>({...entry.item,relevance:entry.relevance})),actions:relatedActions,counts:{objects:rankedObjects.length,relationships:relatedRelationships.length,events:relatedEvents.length,observations:rankedObservations.length,actions:relatedActions.length}};
  }
  ...
  async decideAiActionProposal(workspaceId, proposalId, input, actorId) {
    ...
    const outcome=await this.repository.transaction(async (repository) => {
      ...
      if(calendar){if(typeof repository.lockCalendarSchedule==='function')await repository.lockCalendarSchedule(workspaceId,calendar.targetUserId??actorId);const start=new Date(calendar.startsAt).getTime();const end=new Date(calendar.endsAt).getTime();const events=(await repository.listObjects(workspaceId,{type:'calendar_event'})).filter(item=>!item.deletedAt&&(item.state?.targetUserId??calendar.targetUserId??actorId)===(calendar.targetUserId??actorId));if(events.some(event=>{const eventStart=new Date(event.state?.startsAt??event.state?.start).getTime();const eventEnd=new Date(event.state?.endsAt??event.state?.end).getTime();return Number.isFinite(eventStart)&&Number.isFinite(eventEnd)&&start<eventEnd&&end>eventStart;}))throw new AtlasError('CALENDAR_SLOT_CONFLICT','The calendar time became unavailable before it could be scheduled',409);}
      const templateProvenance=proposal.actionType==='create_document'?proposal.input.templateProvenance??null:null;
      const precedentProvenance=proposal.actionType==='create_document'?proposal.input.precedentProvenance??[]:[];if(!Array.isArray(precedentProvenance)||precedentProvenance.length>12)throw new AtlasError('AI_PRECEDENT_PROVENANCE_INVALID','Reusable-work provenance is invalid',400);
      ...
      const precedentDocuments=[];for(const provenance of precedentProvenance){const document=await repository.getObject(workspaceId,required(provenance.documentId,'precedentProvenance.documentId'));if(document.dimension!=='document'||document.deletedAt)throw new AtlasError('AI_PRECEDENT_DOCUMENT_INVALID','A reusable-work source is no longer an active authorized firm document',409,{documentId:document.id});if(document.version!==provenance.sourceVersion)throw new AtlasError('AI_PRECEDENT_DOCUMENT_VERSION_CONFLICT','A reusable-work source changed after this draft was prepared. Generate a new draft from current sources.',409,{documentId:document.id,expectedVersion:provenance.sourceVersion,currentVersion:document.version});precedentDocuments.push(document);}
      const specifications = {
        ...
        create_document: { dimension: 'document', type: proposal.input.documentType, title: proposal.input.title, state: { content: proposal.input.content, templateData: proposal.input.templateData??null, templateProvenance, precedentProvenance, generationProvenance:proposal.input.generationProvenance??null, sourceMatterVersion: proposal.input.sourceMatterVersion??null, reviewRequired: proposal.input.reviewRequired!==false, status: 'draft', filed: false } },
      ...
      for(const document of precedentDocuments)await repository.createRelationship({id:createId('rel'),workspaceId,fromObjectId:created.id,toObjectId:document.id,type:'derived_from_firm_work_product',attributes:{sourceVersion:document.version,sourceMatterId:document.parentObjectId??document.state?.matterId??null,reusePolicy:'structure_and_language_only'},createdAt:now});
      const relatedSourceIds=[...(sourceTemplate?[sourceTemplate.id]:[]),...precedentDocuments.map(document=>document.id)];await repository.createEvent(this.buildEvent(workspaceId, { parentObjectId: created.id, relatedObjectIds:relatedSourceIds, type: precedentDocuments.length?'document.draft_created_with_reused_work':sourceTemplate?'document.draft_created_from_form':'object.created', actorId, source: 'atlas.ai.approval', data: { objectType: created.type, actionType: proposal.actionType, proposalId,templateId:sourceTemplate?.id??null,templateVersion:templateProvenance?.sourceVersion??null,precedentDocumentIds:precedentDocuments.map(document=>document.id) } }));
