...
test('automatic firm-wide recall combines canonical records with encrypted prior final work without storing chain-of-thought',async()=>{
  const {service,workspace}=await fixture();const cipher={encrypt:(value,context)=>`sealed:${context}:${value}`,decrypt:(value,context)=>String(value).replace(`sealed:${context}:`,'')};const priorId='air_prior_firm_work';await service.repository.createAiRun({id:priorId,workspaceId:workspace.id,actorId:'usr_prior',status:'completed',prompt:cipher.encrypt('Prepare a motion to compel discovery',`run:${priorId}:prompt`),answer:cipher.encrypt('A prior motion outline used a deficiency section and certification section.',`run:${priorId}:answer`),provider:'test',model:'test',sources:[],toolCalls:2,usage:{inputTokens:1,outputTokens:1,totalTokens:2},errorCode:null,createdAt:'2026-07-09T12:00:00.000Z'});let recalled='';const model={async complete(input){recalled=input.messages.filter(message=>message.role==='developer').map(message=>message.content).join('\n');return{text:'I found relevant firm work and will validate it against the current case.'};}};const assistant=new AtlasAssistant(model,new AtlasToolRegistry(service,{contentCipher:cipher}),{repository:service.repository,contentCipher:cipher});const result=await assistant.query({workspaceId:workspace.id,userId:'usr_attorney',prompt:'Prepare a motion to compel using relevant work the firm has done before'});assert.match(recalled,/Automatic authorized firm-wide recall/);assert.match(recalled,/Motion to Compel Discovery/);assert.match(recalled,/prior motion outline/i);assert.match(recalled,/hidden reasoning is never stored/);assert.ok(result.recallSources.some(item=>item.sourceType==='atlas_prior_work'&&item.runId===priorId));const run=(await service.repository.listAiRuns(workspace.id,1))[0];assert.ok(run.sources.some(item=>item.sourceType==='atlas_prior_work'&&item.runId===priorId));assert.equal('chainOfThought' in run,false);
});

test('cross-case work product is searched firm-wide then copied into a new case draft with immutable source provenance',async()=>{
  const {service,workspace,matter}=await fixture();const sourceMatter=await service.createObject(workspace.id,{dimension:'matter',type:'civil',title:'Prior Discovery Case',state:{caseNumber:'OLD-1',courtName:'Prior Court'}});const precedent=await service.createObject(workspace.id,{parentObjectId:sourceMatter.id,dimension:'document',type:'motion',title:'Prior Motion to Compel',state:{matterId:sourceMatter.id,status:'final'}});const cipher={encrypt:(value,context)=>`sealed:${context}:${value}`,decrypt:(value,context)=>String(value).replace(`sealed:${context}:`,'')};await service.repository.createDocumentKnowledgeChunk({id:'dkc_reusable_motion',workspaceId:workspace.id,sourceObjectId:precedent.id,ordinal:0,content:cipher.encrypt('Reusable structure: deficiency, good-faith conference, requested relief.',`document-chunk:dkc_reusable_motion:content`),sourceLocation:{page:3},provider:'local',model:'reuse-model',dimensions:2,embedding:[1,0],createdAt:'2026-07-10T12:00:00.000Z'});const tools=new AtlasToolRegistry(service,{contentCipher:cipher,embeddingProvider:{async embedTexts(){return{vectors:[[1,0]],model:'reuse-model',dimensions:2};}}});const found=await tools.execute('search_reusable_work_product',workspace.id,{query:'motion to compel discovery structure',targetMatterId:matter.id});assert.equal(found.data.results[0].sourceObjectId,precedent.id);assert.equal(found.data.results[0].matterId,sourceMatter.id);assert.match(found.data.results[0].text,/good-faith conference/);assert.equal(found.data.results[0].useAsCurrentCaseFact,false);const proposed=await tools.execute('propose_create_legal_document',workspace.id,{matterId:matter.id,documentType:'motion',title:'Motion to Compel',precedentDocumentIds:[precedent.id],body:'DEFICIENCIES\n[ATTORNEY INPUT REQUIRED: source-supported deficiencies]\n\nGOOD-FAITH CONFERENCE\n[ATTORNEY INPUT REQUIRED: conference facts]\n\nRELIEF REQUESTED\n[ATTORNEY INPUT REQUIRED: requested relief]'});assert.equal(proposed.actionProposal.input.precedentProvenance[0].documentId,precedent.id);const proposal=await service.repository.createAiActionProposal({id:'aap_reused_work',workspaceId:workspace.id,runId:null,intelligenceJobId:null,originType:'chat',proposedBy:'usr_attorney',actionType:'create_document',input:proposed.actionProposal.input,status:'pending',version:1,decidedBy:null,resultObjectId:null,createdAt:'2026-07-10T12:00:00.000Z',decidedAt:null});const approved=await service.decideAiActionProposal(workspace.id,proposal.id,{version:1,decision:'approve'},'usr_attorney');assert.equal(approved.result.parentObjectId,matter.id);assert.equal(approved.result.state.precedentProvenance[0].sourceMatterId,sourceMatter.id);const relationships=await service.repository.listRelationships(workspace.id);assert.ok(relationships.some(relationship=>relationship.fromObjectId===approved.result.id&&relationship.toObjectId===precedent.id&&relationship.type==='derived_from_firm_work_product'));const sourceAfter=await service.getObject(workspace.id,precedent.id);assert.equal(sourceAfter.version,precedent.version);assert.equal(sourceAfter.parentObjectId,sourceMatter.id);
});
