...
test('public website connections bind an ingestion token to one firm and attorney calendar',()=>{const connection={websiteId:'flo-dui',workspaceId:'wsp_firm',targetUserId:'usr_attorney',token:'x'.repeat(32),timeZone:'America/New_York'};assert.deepEqual(loadConfig({ATLAS_PUBLIC_WEBSITE_CONNECTIONS:JSON.stringify([connection])}).publicWebsiteConnections,[connection]);assert.throws(()=>loadConfig({ATLAS_PUBLIC_WEBSITE_CONNECTIONS:JSON.stringify([{...connection,token:'short'}])}),/32-character token/);assert.throws(()=>loadConfig({ATLAS_PUBLIC_WEBSITE_CONNECTIONS:JSON.stringify([connection,{...connection,workspaceId:'wsp_other'}])}),/duplicate websiteId/);});
