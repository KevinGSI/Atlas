...
test('one-click Mac demo rejects stale servers and keeps data outside replaceable source checkouts',async()=>{const launch=await readFile('Launch Atlas Demo.command','utf8');const stop=await readFile('Stop Atlas Demo.command','utf8');assert.match(launch,/\/usr\/bin\/nohup "\$NODE_BIN" src\/server\.js/);assert.match(launch,/\.atlas-local-demo\.pid/);assert.match(launch,/disown "\$SERVER_PID"/);assert.match(launch,/Library\/Application Support\/Atlas Demo/);assert.match(launch,/LOCAL_DATA_PATH=.*DEMO_DATA_DIR\/repository\.bin/);assert.match(launch,/DOCUMENT_STORAGE_PROVIDER=.*filesystem/);assert.match(launch,/DOCUMENT_STORAGE_PATH=.*DEMO_DATA_DIR\/documents/);assert.match(launch,/LEGACY_DATA_DIR=.*\.atlas-data/);assert.match(launch,/\/usr\/bin\/ditto "\$LEGACY_DATA_DIR" "\$DEMO_DATA_DIR"/);assert.match(launch,/DEMO_BACKUP_DIR\/repository-\$SNAPSHOT_TIME\.bin/);assert.doesNotMatch(launch,/rm -rf.*DEMO_DATA_DIR|rm -rf.*ATLAS_DEMO_HOME/);assert.match(launch,/earlier manually started Atlas server/);assert.match(launch,/prevents the browser from silently reopening outdated backend code/);assert.doesNotMatch(launch,/trap stop_atlas/);assert.match(launch,/Atlas will keep running after this window closes/);assert.match(stop,/lsof -nP -tiTCP:3000 -sTCP:LISTEN/);assert.match(stop,/CANDIDATE_CWD.*PROJECT_DIR/);assert.match(stop,/kill "\$SERVER_PID"/);assert.match(stop,/\/bin\/rm -f "\$PID_FILE"/);assert.match(stop,/demo records and uploaded files remain/);assert.doesNotMatch(stop,/rm -rf.*Atlas Demo/);});
...
test('security analysis uses Node 24-compatible maintained action runtimes',async()=>{const source=await readFile('.github/workflows/security-analysis.yml','utf8');assert.match(source,/actions\/checkout@v6/);assert.match(source,/actions\/setup-node@v6/);assert.match(source,/pnpm\/action-setup@v4\.4\.0/);assert.match(source,/github\/codeql-action\/init@v4/);assert.match(source,/github\/codeql-action\/analyze@v4/);assert.doesNotMatch(source,/actions\/(?:checkout|setup-node)@v4/);assert.doesNotMatch(source,/github\/codeql-action\/(?:init|analyze)@v3/);});

test('every push and pull request runs complete Atlas verification',async()=>{const source=await readFile('.github/workflows/core-verification.yml','utf8');const workflow=YAML.parse(source);const job=workflow.jobs.verify;assert.equal(job.steps.at(-1).run,'pnpm verify');assert.equal(job.steps.find((step)=>String(step.uses).startsWith('actions/setup-node')).with['node-version'],24);assert.match(source,/push:/);assert.match(source,/pull_request:/);});

test('GitHub workflows use package.json as the single pnpm version source',async()=>{const pkg=JSON.parse(await readFile('package.json','utf8'));assert.equal(pkg.packageManager,'pnpm@11.7.0');for(const name of ['core-verification','postgres-integration','security-analysis','openai-evaluation','staging-smoke']){const source=await readFile(`.github/workflows/${name}.yml`,'utf8');assert.match(source,/pnpm\/action-setup@v4\.4\.0/);assert.doesNotMatch(source,/\n\s+version:\s*11\./);}});
