...
test('one-click Mac demo launch stays alive after Terminal closes and has a separate stop control',async()=>{const launch=await readFile('Launch Atlas Demo.command','utf8');const stop=await readFile('Stop Atlas Demo.command','utf8');assert.match(launch,/\/usr\/bin\/nohup "\$NODE_BIN" src\/server\.js/);assert.match(launch,/\.atlas-local-demo\.pid/);assert.match(launch,/disown "\$SERVER_PID"/);assert.doesNotMatch(launch,/trap stop_atlas/);assert.match(launch,/Atlas will keep running after this window closes/);assert.match(stop,/kill "\$SERVER_PID"/);assert.match(stop,/\/bin\/rm -f "\$PID_FILE"/);});
