...
test('one-click Mac demo rejects stale manual servers, retains data, and stops only a verified Atlas process',async()=>{const launch=await readFile('Launch Atlas Demo.command','utf8');const stop=await readFile('Stop Atlas Demo.command','utf8');assert.match(launch,/\/usr\/bin\/nohup "\$NODE_BIN" src\/server\.js/);assert.match(launch,/\.atlas-local-demo\.pid/);assert.match(launch,/disown "\$SERVER_PID"/);assert.match(launch,/LOCAL_DATA_PATH=.*\.atlas-data\/repository\.bin/);assert.match(launch,/DOCUMENT_STORAGE_PROVIDER=.*filesystem/);assert.match(launch,/DOCUMENT_STORAGE_PATH=.*\.atlas-data\/documents/);assert.match(launch,/earlier manually started Atlas server/);assert.match(launch,/prevents the browser from silently reopening outdated backend code/);assert.doesNotMatch(launch,/trap stop_atlas/);assert.match(launch,/Atlas will keep running after this window closes/);assert.match(stop,/lsof -nP -tiTCP:3000 -sTCP:LISTEN/);assert.match(stop,/CANDIDATE_CWD.*PROJECT_DIR/);assert.match(stop,/kill "\$SERVER_PID"/);assert.match(stop,/\/bin\/rm -f "\$PID_FILE"/);});
