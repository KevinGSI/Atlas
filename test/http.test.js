...
test('frontend uses a versioned deferred script that executes in the local pilot browser',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/<span class="sr-only">Reply to Atlas or ask anything<\/span><textarea/);assert.match(page.body,/placeholder="Reply to Atlas or ask anything…"/);assert.match(page.body,/<script defer src="\.\/app\.js\?v=1\.0\.0-rc\.1-microsoft365-calendar-1"><\/script>/);});
...
test('Accounting exposes a read-only QuickBooks connection and canonical financial mirror',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/microsoft365-calendar-1/);const script=await raw(handler,'/app.js');assert.match(script.body,/QuickBooks Online/);assert.match(script.body,/Intuit’s secure consent screen/);assert.match(script.body,/cms\/quickbooks\/authorize/);assert.match(script.body,/quickBooksSync/);assert.match(script.body,/QuickBooks finances are mirrored read-only/);assert.match(script.body,/externalSource/);});
...
test('Microsoft 365 calendar events share the mailbox connection and render as canonical Calendar records',async()=>{const handler=fixture();const script=await raw(handler,'/app.js');assert.match(script.body,/calendar:\{title:'Calendar'.*type:'calendar_event'.*readOnly:true/);assert.match(script.body,/\['calendar_event','deadline'\]\.includes\(object\.type\)/);assert.match(script.body,/item\.state\?\.startsAt\?\?item\.state\?\.start/);assert.match(script.body,/Sync email & calendar/);assert.match(script.body,/calendar events/);assert.match(script.body,/Open in Microsoft 365/);assert.match(script.body,/function safeExternalUrl\(value\)/);assert.match(script.body,/window\.open\(authorizationUrl\.href,'atlas-mailbox-authorization','popup,width=720,height=820'\)/);assert.match(script.body,/popup\.opener=null/);});
