...
test('frontend uses a versioned deferred script that executes in the local pilot browser',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/<script defer src="\.\/app\.js\?v=0\.36\.0-11"><\/script>/);});
...
test('homepage automatically presents the latest canonical firm activity as an actionable hub',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/Recent Activity/);assert.match(page.body,/id="recentActivityStatus"/);assert.match(page.body,/id="recentActivityList"/);assert.match(page.body,/Canonical firm activity/);const script=await raw(handler,'/app.js');assert.match(script.body,/function renderRecentActivity\(events\)/);assert.match(script.body,/sort\(\(a,b\)=>new Date\(b\.occurredAt\)-new Date\(a\.occurredAt\)\)/);assert.match(script.body,/slice\(0,10\)/);assert.match(script.body,/Open case/);assert.match(script.body,/\/events`/);assert.doesNotMatch(page.body,/Show activity since/);});
