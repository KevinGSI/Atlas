...
test('frontend uses a versioned deferred script that executes in the local pilot browser',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/<script defer src="\.\/app\.js\?v=0\.36\.0-8"><\/script>/);});
...
test('non-home authenticated views carry the shared digital-twin dock while Home uses only its command center',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/id="twinDock"/);assert.match(page.body,/id="twinForm"/);assert.match(page.body,/Ask Atlas from anywhere/);assert.match(page.body,/aria-label="Atlas digital twin"/);const script=await raw(handler,'/app.js');assert.match(script.body,/submitTwinCommand/);assert.match(script.body,/conversationId/);assert.match(script.body,/assistant\/query/);assert.match(script.body,/actionProposals/);assert.match(script.body,/twinToggle/);assert.match(script.body,/twinDock'\)\.classList\.toggle\('hidden',name==='dashboard'/);});
