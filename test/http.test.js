...
test('frontend uses a versioned deferred script that executes in the local pilot browser',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/<script defer src="\.\/app\.js\?v=0\.36\.0-13"><\/script>/);});

test('homepage summary controls are recognizable buttons with relevant working destinations',async()=>{const handler=fixture();const page=await raw(handler,'/');for(const id of ['openCasesMetric','openTasksMetric','reviewDeadlinesMetric','openReviewMetric'])assert.match(page.body,new RegExp(`id="${id}"`));assert.match(page.body,/aria-label="Open all cases"/);assert.match(page.body,/View task workspace/);assert.match(page.body,/Ask Atlas to review/);assert.match(page.body,/Open review inbox/);assert.match(page.body,/id="reviewInbox"/);const script=await raw(handler,'/app.js');assert.match(script.body,/openCasesMetric.*showView\('matters'\)/);assert.match(script.body,/openTasksMetric.*showView\('tasks'\)/);assert.match(script.body,/reviewDeadlinesMetric.*submitCommand\('Review every approaching or missed deadline/);assert.match(script.body,/openReviewMetric.*reviewInbox.*scrollIntoView/);});

test('Legal Assistant is a dedicated menu workspace backed by the same native firm twin',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/data-view="dashboard">Home<\/button><button data-view="legal-assistant">Legal Assistant<\/button><button data-view="matters">Cases/);assert.match(page.body,/id="view-legal-assistant"/);assert.match(page.body,/id="legalAssistantForm"/);assert.match(page.body,/id="legalAssistantTranscript"/);assert.match(page.body,/authorized knowledge and workflows of your firm/);const script=await raw(handler,'/app.js');assert.match(script.body,/function submitLegalAssistant\(prompt\)/);assert.match(script.body,/legalAssistant.*assistant\/query/s);assert.match(script.body,/conversationId=result\.conversationId/);assert.match(script.body,/result\.sources/);assert.match(script.body,/result\.actionProposals/);assert.match(script.body,/name==='legal-assistant'\?'Legal Assistant'/);});
