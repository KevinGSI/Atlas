...
test('serves a local visual template studio with modern samples and fully editable design settings',async()=>{const handler=fixture();const page=await raw(handler,'/template-editor');assert.equal(page.status,200);assert.match(page.headers['content-type'],/text\/html/);assert.match(page.body,/Atlas Template Studio/);assert.match(page.body,/contenteditable="true"/);assert.match(page.body,/Save template/);assert.match(page.body,/Download JSON/);assert.match(page.body,/Import JSON/);assert.match(page.body,/Reset defaults/);assert.match(page.body,/First panel/);for(const name of ['Harbor','Kinetic','Nocturne','Editorial'])assert.match(page.body,new RegExp(`>${name}<`));for(const control of ['sidebarText','accentSoft','surface','headingFont','layout','heroStyle','navStyle','cardStyle','sidebarWidth'])assert.match(page.body,new RegExp(`name="${control}"`));for(const editable of ['firmPlan','systemStatus','readinessOne','suggestionFour','inboxBadge','signalFourLabel','accountLabel'])assert.match(page.body,new RegExp(`data-edit="${editable}"`));const script=await raw(handler,'/template-editor.js');assert.equal(script.status,200);assert.match(script.headers['content-type'],/javascript/);assert.match(script.body,/localStorage/);assert.match(script.body,/atlas-template-settings\.json/);assert.match(script.body,/panelOrder/);assert.match(script.body,/navigator\.clipboard/);assert.match(script.body,/const presets=/);assert.match(script.body,/applyPreset/);});

test('applies the approved Harbor template settings to the working Atlas interface',async()=>{const handler=fixture();const [page,script]=await Promise.all([raw(handler,'/'),raw(handler,'/app.js')]);assert.match(page.body,/id="atlas-selected-template"/);for(const value of ['#102a43','#eff6ff','#2f6fed','#eaf1ff','#eef3f8','#ffffff','#122033','#617188','#d5deea','#1b8a63'])assert.match(page.body,new RegExp(value));assert.match(page.body,/grid-template-columns:228px minmax\(0,1fr\)/);assert.match(page.body,/--atlas-radius:18px/);assert.match(page.body,/--atlas-heading:Iowan Old Style/);assert.match(script.body,/function applySelectedAtlasTemplate\(\)/);for(const copy of ['Good morning, Kevin.','Atlas command workspace','Ask or request anything…','Summarize recent cases','Find motions to compel','Prepare legal work','Review queue','Active tasks'])assert.match(script.body,new RegExp(copy.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));});

test('frontend uses a versioned deferred script that executes in the local pilot browser',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/<span class="sr-only">Reply to Atlas or ask anything<\/span><textarea/);assert.match(page.body,/placeholder="Reply to Atlas or ask anything…"/);assert.match(page.body,/<script defer src="\.\/app\.js\?v=1\.0\.0-rc\.1-communication-1"><\/script>/);});

test('Documents provides real secure upload, automatic AI cataloging, and verified download controls',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/id="collectionFile" type="file"/);assert.match(page.body,/application\/pdf/);const script=await raw(fixture(),'/app.js');assert.match(script.body,/contentBase64/);assert.match(script.body,/\/files/);assert.match(script.body,/Download verified file/);assert.match(script.body,/\/content/);assert.match(script.body,/monitorDocumentAnalysis/);assert.match(script.body,/appendDocumentAnalysis/);assert.match(script.body,/Atlas is now reading, classifying, summarizing, and cataloging the document/);});

test('Documents includes a separate firm-wide Form Bank and case-grounded drafting controls',async()=>{const script=(await raw(fixture(),'/app.js')).body;assert.match(script,/function installDocumentWorkspaceNavigation\(\)/);assert.match(script,/Case Documents/);assert.match(script,/Form Bank/);assert.match(script,/Firm-wide forms not associated with a case/);assert.match(script,/Sign & Notarize/);assert.match(script,/function showDocumentSection\(section\)/);assert.match(script,/function installFormBankPanel\(\)/);assert.match(script,/id="formBankUploadForm"/);assert.match(script,/Drop a form here/);assert.match(script,/DOCX, PDF, or TXT/);assert.match(script,/formBankSearch/);assert.match(script,/\/form-bank/);assert.match(script,/contentBase64/);assert.match(script,/Download verified file/);assert.match(script,/archive/);assert.match(script,/restore/);assert.match(script,/Use a Form Bank form/);assert.match(script,/form-bank\/\$\{encodeURIComponent\(formId\)\}\/drafts/);assert.match(script,/Required drafting instructions/);assert.match(script,/nothing was sent or filed/i);});

test('case Documents accepts computer selection and drag-drop through one case-scoped storage path',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/data-matter-tab="documents"/);assert.match(page.body,/\.document-drop-zone/);const script=await raw(handler,'/app.js');assert.match(script.body,/id="matterDocumentDropZone"/);assert.match(script.body,/id="matterDocumentFiles"[^>]+multiple/);assert.match(script.body,/\['dragenter','dragover'\]/);assert.match(script.body,/event\.dataTransfer\.files/);assert.match(script.body,/function uploadMatterDocuments\(\)/);assert.match(script.body,/matterId,filename:file\.name/);assert.match(script.body,/\/files`/);assert.match(script.body,/monitorMatterDocumentAnalysis/);assert.match(script.body,/function documentProvenance\(item\)/);assert.match(script.body,/Pulled from \$\{provider\}/);assert.match(script.body,/Uploaded securely from a computer/);assert.match(script.body,/Download verified file/);});

test('case Communications provides contact call, text, email, and meeting actions without silently sending',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/data-matter-tab="communications"/);assert.match(page.body,/\.matter-communication-actions/);const script=await raw(handler,'/app.js');for(const label of ['Call','Text','Email','Set up meeting'])assert.match(script.body,new RegExp(`matterCommunicationAction\\('${label}'`));assert.match(script.body,/matters\/\$\{encodeURIComponent\(matterId\)\}\/client-communications/);assert.match(script.body,/path='text-drafts'/);assert.match(script.body,/path='email-drafts'/);assert.match(script.body,/path='meeting-drafts'/);assert.match(script.body,/window\.location\.href=result\.dialUri/);assert.match(script.body,/Nothing was sent/);assert.match(script.body,/no calendar event was created/i);for(const type of ['incoming_email','email_draft','sms_message','sms_draft','client_call_attempt'])assert.match(script.body,new RegExp(`'${type}'`));});

test('case Communications wires a canonical contact selector into each call, text, email, and meeting action',async()=>{const script=(await raw(fixture(),'/app.js')).body;assert.match(script,/communicationRecipient/);assert.match(script,/select\.dataset\.communicationRecipient=mode/);for(const mode of ['call','text','email','meeting'])assert.match(script,new RegExp(`matterCommunicationAction\\('[^']*','${mode}'`));for(const label of ['Client','Opposing counsel','Judicial assistant','Expert witness','Other contact'])assert.match(script,new RegExp(label));assert.match(script,/contacts/);assert.match(script,/capabilities/);assert.match(script,/contactId/);assert.match(script,/JSON\.stringify\(\{contactId/);assert.match(script,/body=\{contactId/);assert.match(script,/Recipient for/);});

test('Documents includes searchable Docusign and Virtual Notary workflows without impersonating a signer or notary',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/id="documentExecutionPanel"/);assert.match(page.body,/Docusign/);assert.match(page.body,/Virtual Notary/);assert.match(page.body,/Find a PDF or DOCX/);assert.match(page.body,/Atlas never applies another person’s signature/);assert.match(page.body,/A commissioned notary—not Atlas—performs the act/);const script=await raw(fixture(),'/app.js');assert.match(script.body,/document-execution\/status/);assert.match(script.body,/document-execution\/documents/);assert.match(script.body,/document-execution\/signatures/);assert.match(script.body,/document-execution\/notarizations/);assert.match(script.body,/Use for Docusign/);assert.match(script.body,/completedDocumentId/);assert.match(script.body,/window\.open\('about:blank','atlas-docusign'/);});

test('document execution HTTP boundaries separate authenticated selection from signed provider completion',async()=>{const calls=[];const documentExecution={status(workspaceId){calls.push(['status',workspaceId]);return{docusignConfigured:true,virtualNotaryConfigured:false};},async documents(workspaceId,input){calls.push(['documents',workspaceId,input]);return[{id:'obj_doc'}];},async requestSignature(workspaceId,input,userId){calls.push(['signature',workspaceId,input,userId]);return{request:{id:'obj_request'},launchUrl:'https://docusign.example'};},async requestNotarization(){throw new Error('not used');},async processSignatureWebhook(workspaceId,provider,raw,signature){calls.push(['webhook',workspaceId,provider,raw,signature]);return{request:{id:'obj_request',state:{status:'completed'}}};}};const permissions=[];const identity={async authenticate(){return{id:'usr_attorney'};},async authorize(workspaceId,userId,permission){permissions.push(permission);}};const handler=createAtlasHandler(new AtlasService(new InMemoryRepository()),{config:{maxBodyBytes:1_048_576,corsOrigins:[]},ready:async()=>true,identity,documentExecution});const headers={authorization:'Bearer test'};assert.equal((await json(handler,'/v1/workspaces/wsp_1/document-execution/status',{headers})).status,200);assert.deepEqual(calls[0],['status','wsp_1']);assert.equal((await json(handler,'/v1/workspaces/wsp_1/document-execution/documents?query=motion&matterId=obj_case',{headers})).status,200);assert.equal((await json(handler,'/v1/workspaces/wsp_1/document-execution/signatures',{method:'POST',headers,body:JSON.stringify({documentId:'obj_doc'})})).status,201);const webhook=await json(handler,'/v1/document-execution/docusign/wsp_1/webhook',{method:'POST',headers:{'x-docusign-signature-1':'signed'},body:'{"event":"envelope-completed"}'});assert.equal(webhook.status,200);assert.deepEqual(permissions,['workspace:read','workspace:read','workspace:write']);assert.deepEqual(calls.at(-1),['webhook','wsp_1','docusign','{"event":"envelope-completed"}','signed']);});

test('case contact communication HTTP routes forward contactId while preserving case binding and authorization',async()=>{
  const calls=[];
  const caseCommunications={
    async status(workspaceId,matterId){calls.push(['status',workspaceId,matterId]);return{contacts:[{id:'obj_contact',role:'opposing_counsel'}]};},
    async prepareCall(workspaceId,matterId,input,userId){calls.push(['call',workspaceId,matterId,input,userId]);return{dialUri:'tel:+15550102026'};},
    async createTextDraft(workspaceId,matterId,input,userId){calls.push(['text',workspaceId,matterId,input,userId]);return{id:'obj_text'};},
    async createEmailDraft(workspaceId,matterId,input,userId){calls.push(['email',workspaceId,matterId,input,userId]);return{id:'obj_email'};},
    async createMeetingDraft(workspaceId,matterId,input,userId){calls.push(['meeting',workspaceId,matterId,input,userId]);return{id:'obj_meeting'};}
  };
  const permissions=[];const identity={async authenticate(){return{id:'usr_attorney'};},async authorize(workspaceId,userId,permission){permissions.push([workspaceId,userId,permission]);}};
  const handler=createAtlasHandler(new AtlasService(new InMemoryRepository()),{config:{maxBodyBytes:1_048_576,corsOrigins:[]},ready:async()=>true,identity,caseCommunications});
  const headers={authorization:'Bearer test','content-type':'application/json'};const base='/v1/workspaces/wsp_1/matters/obj_case/client-communications';const contactId='obj_contact';
  assert.equal((await json(handler,base,{headers})).status,200);
  assert.equal((await json(handler,`${base}/call`,{method:'POST',headers,body:JSON.stringify({contactId})})).status,200);
  assert.equal((await json(handler,`${base}/text-drafts`,{method:'POST',headers,body:JSON.stringify({contactId,body:'Draft'})})).status,201);
  assert.equal((await json(handler,`${base}/email-drafts`,{method:'POST',headers,body:JSON.stringify({contactId,instructions:'Update'})})).status,201);
  assert.equal((await json(handler,`${base}/meeting-drafts`,{method:'POST',headers,body:JSON.stringify({contactId,meetingType:'phone',proposedSlots:['2026-07-21T14:00:00.000Z','2026-07-21T16:00:00.000Z']})})).status,201);
  assert.deepEqual(permissions.map(item=>item[2]),['workspace:read','workspace:write','workspace:write','workspace:write','workspace:write']);
  assert.deepEqual(calls.map(item=>item.slice(0,3)),[['status','wsp_1','obj_case'],['call','wsp_1','obj_case'],['text','wsp_1','obj_case'],['email','wsp_1','obj_case'],['meeting','wsp_1','obj_case']]);
  assert.ok(calls.slice(1).every(call=>call.at(-1)==='usr_attorney'));assert.ok(calls.slice(1).every(call=>call.at(-2).contactId===contactId));
});

test('marketing HTTP routes preserve firm read and write boundaries',async()=>{const calls=[];const marketing={async status(workspaceId){calls.push(['status',workspaceId]);return{campaigns:[],signals:[]};},async scan(workspaceId,input,userId){calls.push(['scan',workspaceId,input,userId]);return{signals:[]};},async createCampaign(workspaceId,input,userId){calls.push(['campaign',workspaceId,input,userId]);return{id:'obj_campaign'};},async prepareCampaign(workspaceId,campaignId,input,userId){calls.push(['prepare',workspaceId,campaignId,input,userId]);return{launches:[]};}};const permissions=[];const identity={async authenticate(){return{id:'usr_attorney'};},async authorize(workspaceId,userId,permission){permissions.push(permission);}};const handler=createAtlasHandler(new AtlasService(new InMemoryRepository()),{config:{maxBodyBytes:1_048_576,corsOrigins:[]},ready:async()=>true,identity,marketing});const headers={authorization:'Bearer test','content-type':'application/json'};assert.equal((await json(handler,'/v1/workspaces/wsp_1/marketing',{headers})).status,200);assert.equal((await json(handler,'/v1/workspaces/wsp_1/marketing/public-scans',{method:'POST',headers,body:JSON.stringify({eventTypes:['car_accidents']})})).status,201);assert.equal((await json(handler,'/v1/workspaces/wsp_1/marketing/campaigns',{method:'POST',headers,body:JSON.stringify({title:'Campaign'})})).status,201);assert.equal((await json(handler,'/v1/workspaces/wsp_1/marketing/campaigns/obj_campaign/prepare',{method:'POST',headers,body:JSON.stringify({platformPoliciesReviewed:true})})).status,200);assert.deepEqual(permissions,['workspace:read','workspace:write','workspace:write','workspace:write']);assert.deepEqual(calls.at(-1),['prepare','wsp_1','obj_campaign',{platformPoliciesReviewed:true},'usr_attorney']);});

test('Marketing workspace exposes aggregate public signals and paused social advertising controls',async()=>{const page=await raw(fixture(),'/');const script=await raw(fixture(),'/app.js');assert.match(script.body,/dataset\.view='marketing'/);assert.match(script.body,/Public trends, never named leads/);assert.match(script.body,/data-marketing-event="arrests"/);assert.match(script.body,/data-marketing-event="dissolution_petitions"/);assert.match(script.body,/data-marketing-event="car_accidents"/);assert.match(script.body,/Prepare paused provider campaign/);assert.match(script.body,/attorneyAdvertisingRulesReviewed/);assert.match(script.body,/named audience/);assert.match(page.body,/app\.js\?v=1\.0\.0-rc\.1-communication-1/);});

test('case and firm-wide Tasks let Atlas perform a selected canonical task and render reviewable work inline',async()=>{const script=(await raw(fixture(),'/app.js')).body;assert.match(script,/Have Atlas perform task/);assert.match(script,/performCaseTask\(task,matter,button,host\)/);assert.match(script,/renderTaskExecutionResult/);assert.match(script,/matters\/\$\{encodeURIComponent\(matter\.id\)\}\/tasks\/\$\{encodeURIComponent\(task\.id\)\}\/perform/);assert.match(script,/appendWorkQueueItem\(list,item,\{allowAtlasExecution:true\}\)/);assert.match(script,/options\.allowAtlasExecution&&item\.kind==='task'\)appendTaskExecution\(card,item\.raw,item\.matter\)/);assert.match(script,/Nothing will be sent or filed/);assert.match(script,/result\.actionProposals/);});

test('every case subsection exposes working canonical creation and Atlas action controls',async()=>{const script=(await raw(fixture(),'/app.js')).body;assert.match(script,/const caseActionTabs=new Set\(\['contacts','documents','communications','tasks','deadlines','billing'\]\)/);assert.match(script,/Add client or contact/);assert.match(script,/Upload documents/);assert.match(script,/Log a communication/);assert.match(script,/Add task and have Atlas perform/);assert.match(script,/Have Atlas review case deadlines/);assert.match(script,/Add time/);assert.match(script,/Add expense/);assert.match(script,/Create draft invoice/);assert.match(script,/Have Atlas review billing work/);assert.match(script,/accounting\/time-entries/);assert.match(script,/accounting\/expenses/);assert.match(script,/accounting\/invoices/);assert.match(script,/assistant\/query/);assert.match(script,/caseActionResults/);assert.match(script,/renderMatterTabWithCaseActions/);});

test('each case Tasks subsection includes an open case-bound Atlas conversation and identified-task review queue',async()=>{const script=(await raw(fixture(),'/app.js')).body;assert.match(script,/function renderCaseTaskAssistant\(panel\)/);assert.match(script,/What do you need\?/);assert.match(script,/Ask or request anything for this case…/);assert.match(script,/This case only/);assert.match(script,/function submitCaseTaskChat\(event,matterId\)/);assert.match(script,/contextObjectId:matterId/);assert.match(script,/state\.conversationId\?\{conversationId:state\.conversationId\}/);assert.match(script,/Atlas-identified tasks awaiting review/);assert.match(script,/action\.actionType==='create_task'/);assert.match(script,/renderAction\(action\)/);assert.match(script,/event\.key==='Enter'&&!event\.shiftKey&&!event\.isComposing/);assert.match(script,/nothing is sent or filed automatically/i);});
...
test('Accounting exposes a read-only QuickBooks connection and canonical financial mirror',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/communication-1/);const script=await raw(handler,'/app.js');assert.match(script.body,/QuickBooks Online/);assert.match(script.body,/Intuit’s secure consent screen/);assert.match(script.body,/cms\/quickbooks\/authorize/);assert.match(script.body,/quickBooksSync/);assert.match(script.body,/QuickBooks finances are mirrored read-only/);assert.match(script.body,/externalSource/);});

test('Accounting provides ten routed billing pages with working canonical forms and client trust safeguards',async()=>{const script=(await raw(fixture(),'/app.js')).body;for(const label of ['Dashboard','Time entries','Expenses','Requested funds','Invoices','Legacy Reconciliation','Payment plans','Account activity','Trust accounting','Billing guide'])assert.match(script,new RegExp(label));for(const page of ['dashboard','time-entries','expenses','requested-funds','invoices','legacy-reconciliation','payment-plans','account-activity','trust-accounting','billing-guide'])assert.match(script,new RegExp(`data-accounting-page-link="\\$\\{page\\}"|\\['${page}'`));for(const form of ['timeEntryForm','expenseForm','fundRequestForm','billingInvoiceForm','legacyReconciliationForm','paymentPlanForm','trustTransactionForm'])assert.match(script,new RegExp(`id="${form}"`));for(const endpoint of ['time-entries','expenses','fund-requests','invoices','legacy-reconciliations','payment-plans','trust-transactions'])assert.match(script,new RegExp(`submitAccountingRecord\\(event\\.target,'[^']+','${endpoint}'`));assert.match(script,/showAccountingPage/);assert.match(script,/#accounting/);assert.match(script,/prevents disbursement beyond that client’s recorded balance/);});

test('Accounting has an expandable persistent sidebar menu linking every billing page from anywhere',async()=>{const script=(await raw(fixture(),'/app.js')).body;assert.match(script,/function installAccountingSidebarMenu\(\)/);assert.match(script,/sidebarAccountingMenu/);assert.match(script,/Object\.entries\(accountingPageLabels\)/);assert.match(script,/accountingSidebarPage=page/);assert.match(script,/showView\('accounting'\);showAccountingPage\(page,true\)/);assert.match(script,/aria-expanded/);assert.match(script,/sidebar-accounting-menu/);assert.match(script,/data-accounting-sidebar-page/);});

test('Account Info replaces direct sign out and provides a complete account destination',async()=>{const handler=fixture();const page=(await raw(handler,'/')).body;const script=(await raw(handler,'/app.js')).body;assert.match(page,/id="accountInfo" type="button">Account Info<\/button>/);assert.doesNotMatch(page,/id="signOut"/);for(const label of ['My profile &amp; contact info','Settings','Support Center','Make a suggestion','Refer a friend','Privacy Policy','Terms','Log out'])assert.match(script,new RegExp(label.replace('&amp;','&amp;')));assert.match(script,/id='view-account'/);assert.match(script,/accountAvatar/);assert.match(script,/request\('\/v1\/me'\)/);assert.match(script,/accountLogOut.*signOut/);assert.match(script,/data-account-link/);assert.match(script,/showAccountPage/);});

test('requested funds, payment plans, and legacy reconciliation cross authenticated HTTP write boundaries',async()=>{const calls=[];const accounting={async createFundRequest(workspaceId,input,userId){calls.push(['funds',workspaceId,input,userId]);return {id:'obj_funds'};},async createPaymentPlan(workspaceId,input,userId){calls.push(['plan',workspaceId,input,userId]);return {id:'obj_plan'};},async recordLegacyReconciliation(workspaceId,input,userId){calls.push(['legacy',workspaceId,input,userId]);return {id:'obj_payment'};}};const permissions=[];const identity={async authenticate(){return {id:'usr_billing'};},async authorize(workspaceId,userId,permission){permissions.push([workspaceId,userId,permission]);}};const handler=createAtlasHandler(new AtlasService(new InMemoryRepository()),{accounting,identity,ready:async()=>true,config:{maxBodyBytes:1_048_576,corsOrigins:[]}});const headers={authorization:'Bearer test'};assert.equal((await json(handler,'/v1/workspaces/wsp_accounting/accounting/fund-requests',{method:'POST',headers,body:JSON.stringify({matterId:'obj_matter',amountMinor:10000,title:'Funds'})})).status,201);assert.equal((await json(handler,'/v1/workspaces/wsp_accounting/accounting/payment-plans',{method:'POST',headers,body:JSON.stringify({invoiceId:'obj_invoice',installmentCount:4,frequency:'monthly'})})).status,201);assert.equal((await json(handler,'/v1/workspaces/wsp_accounting/accounting/legacy-reconciliations',{method:'POST',headers,body:JSON.stringify({invoiceId:'obj_invoice',amountMinor:10000,rail:'ach',externalReference:'old-1'})})).status,200);assert.deepEqual(calls.map(item=>item[0]),['funds','plan','legacy']);assert.deepEqual(permissions.map(item=>item[2]),['workspace:write','workspace:write','workspace:write']);});
...
test('Phone Assistant is removed from the authenticated client',async()=>{const handler=fixture();const page=await raw(handler,'/');const script=await raw(handler,'/app.js');assert.doesNotMatch(page.body,/data-view="voice"/);assert.doesNotMatch(page.body,/id="view-voice"/);assert.doesNotMatch(page.body,/Phone Assistant/);assert.doesNotMatch(script.body,/loadVoiceAssistant|voiceConfigurationForm|voiceSimulatorForm|showVoiceSimulation/);});
test('Accounting retains the lower-sidebar icon treatment',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/\.nav button\[data-view="accounting"\]:before\{content:'\$'\}/);assert.doesNotMatch(page.body,/\.nav button\[data-view="voice"\]/);assert.doesNotMatch(page.body,/\.nav button\[data-view="migration"\]/);});
...
test('Migration lives prominently inside Settings while preserving provider coexistence and previewed imports',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.doesNotMatch(page.body,/data-view="migration">Migration/);assert.match(page.body,/id="view-migration"/);assert.match(page.body,/Connect your current provider/);assert.match(page.body,/Preview migration/);assert.match(page.body,/Previewing never changes firm data/);const script=await raw(handler,'/app.js');assert.match(script.body,/function installSettingsMigration\(\)/);assert.match(script.body,/migration\.id='settingsMigration'/);assert.match(script.body,/settings\.firstElementChild\.after\(migration\)/);assert.match(script.body,/settingsMigrationShortcut/);assert.match(script.body,/Migration & CMS Connections/);assert.match(script.body,/Legal CMS connections/);assert.match(script.body,/providerNames=\[\.\.\.new Set\(\['clio','mycase',\.\.\.migrationProviders/);assert.match(script.body,/nonLegalProviders=new Set\(\['google','microsoft','quickbooks'\]\)/);assert.match(script.body,/if\(name==='settings'\).*loadMigration\(\)/);assert.doesNotMatch(script.body,/if\(name==='migration'\)/);assert.match(script.body,/migration\/preview/);assert.match(script.body,/migration\/imports/);assert.match(script.body,/cms\/connections/);assert.match(script.body,/selectedMigrationPayload/);});

test('Home uses the structurally distinct AI Command Center application template',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/class="view command-center"/);assert.match(page.body,/class="command-deck"/);assert.match(page.body,/AI Command Center/);assert.match(page.body,/Atlas Command/);assert.match(page.body,/Firm knowledge connected/);assert.match(page.body,/Canonical work synchronized/);assert.match(page.body,/Human approval enforced/);assert.match(page.body,/class="command-center-grid"/);assert.match(page.body,/class="card command command-console flowing-chat"/);assert.match(page.body,/Primary AI workspace/);assert.match(page.body,/class="command-rail"/);assert.match(page.body,/Approval queue/);assert.match(page.body,/class="card review-center"/);assert.match(page.body,/\.nav button\[data-view="workspace"\]:before\{content:'✦'\}/);assert.doesNotMatch(page.body,/Recent Activity/);});

test('Home places Firm Health and Conflict Alerts at the bottom and exposes canonical legal drafting',async()=>{const handler=fixture();const script=await raw(handler,'/app.js');assert.match(script.body,/function installLegalDraftingExperience\(\)/);assert.match(script.body,/dashboard\.append\(firmHealth,conflicts\)/);assert.match(script.body,/textContent='Firm Health'/);assert.match(script.body,/Draft legal document/);assert.match(script.body,/matterCourtName/);assert.match(script.body,/matterCourtJurisdiction/);assert.match(script.body,/matterJudgeName/);assert.match(script.body,/courtName:byId\('matterCourtName'\)/);});
...
test('Attorney inbox matches What do you need and safely wraps all review content',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/#view-dashboard>\.command-center-grid\{grid-template-columns:repeat\(2,minmax\(0,1fr\)\);align-items:stretch\}/);assert.match(page.body,/#view-dashboard>\.command-center-grid>\.command-console\{grid-column:1;grid-row:1;height:100%\}/);assert.match(page.body,/#view-dashboard>\.command-center-grid>\.command-rail>#reviewInbox\{grid-column:2;grid-row:1;display:flex;min-height:0;height:100%;flex-direction:column\}/);assert.match(page.body,/#view-dashboard>\.command-center-grid>\.command-rail>#reviewInbox \.feed\{flex:1;min-height:0;max-height:390px;overflow-y:auto;scrollbar-gutter:stable\}/);assert.match(page.body,/#reviewInbox \.item h3[^}]+overflow-wrap:anywhere;word-break:break-word/);assert.match(page.body,/#reviewInbox \.proposal pre\{white-space:pre-wrap;overflow-x:hidden\}/);assert.match(page.body,/@media\(max-width:1050px\)\{\s*#view-dashboard>\.command-center-grid\{grid-template-columns:minmax\(0,1fr\)\}/);});
test('While You Were Gone uses readable cards linked to canonical cases and source records',async()=>{const handler=fixture();const [page,script]=await Promise.all([raw(handler,'/'),raw(handler,'/app.js')]);assert.match(page.body,/id="atlas-awareness-cards"/);assert.match(page.body,/\.awareness-card-body\{display:grid;gap:10px;padding:15px\}/);assert.match(page.body,/\.awareness-subject\{display:flex/);assert.match(page.body,/\.awareness-prepared \.proposal pre\{max-height:120px/);assert.match(script.body,/function awarenessSubject\(item\)/);assert.match(script.body,/function appendAwarenessSubject\(container,item\)/);assert.match(script.body,/void openSearchObject\(source\)/);assert.match(script.body,/container\.append\(caseLink\(matter\)\)/);assert.match(script.body,/Prepared for your approval/);assert.match(script.body,/Why Atlas flagged this/);assert.match(script.body,/function updateAwarenessSummary\(items\)/);assert.match(script.body,/!\['reviewed','dismissed'\]\.includes\(item\.reviewStatus\)/);});
test('every search bar offers linked familiar Atlas destinations',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/app\.js\?v=1\.0\.0-rc\.1-communication-1/);const script=await raw(handler,'/app.js');assert.match(script.body,/function installAtlasSearchAutocomplete\(\)/);assert.match(script.body,/input\[type="search"\]/);assert.match(script.body,/Familiar Atlas destinations/);assert.match(script.body,/function matchingAtlasDestinations\(query\)/);assert.match(script.body,/function openSearchObject\(object\)/);assert.match(script.body,/destination\.activate\(\)/);assert.match(script.body,/new MutationObserver/);assert.match(script.body,/aria-autocomplete/);assert.match(script.body,/ArrowDown/);assert.match(script.body,/accountingPageLabels/);});
...
test('one-click local preview resumes one durable fictional firm and seeds its case only once',async()=>{const script=await raw(fixture(),'/app.js');assert.match(script.body,/demo@atlas\.local/);assert.match(script.body,/v1\/auth\/login/);assert.match(script.body,/if\(!objects\.some\(item=>item\.dimension==='matter'\)\)await seedDemoCase/);assert.match(script.body,/Morgan v\. Lakeside Property Group/);assert.match(script.body,/DEMO-2026-CV-101/);assert.match(script.body,/Superior Court of the State of Delaware/);assert.match(script.body,/New Castle County/);assert.match(script.body,/Taylor Morgan/);assert.match(script.body,/Jordan Blake \(fictional\)/);assert.match(script.body,/Avery Chambers \(fictional\)/);assert.match(script.body,/Dr\. Riley Chen \(fictional\)/);assert.match(script.body,/Morgan Lee, Court Reporter \(fictional\)/);assert.match(script.body,/Initial Complaint/);assert.match(script.body,/Client intake call/);assert.match(script.body,/Review discovery plan/);assert.match(script.body,/Discovery responses due/);});

test('Settings remains an accessible gear while firm user administration lives in Account Info',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.doesNotMatch(page.body,/data-view="settings">Settings/);assert.match(page.body,/id="settingsButton"/);assert.match(page.body,/aria-label="Settings"/);assert.match(page.body,/>⚙<\/button><button id="accountInfo"[^>]*>Account Info<\/button>/);assert.match(page.body,/id="view-settings"/);assert.match(page.body,/Account Info → Manage Users/);assert.doesNotMatch(page.body,/id="inviteMemberForm"/);const script=await raw(handler,'/app.js');assert.match(script.body,/settingsButton/);assert.match(script.body,/showView\('settings'\)/);assert.match(script.body,/Manage Users/);assert.match(script.body,/dataset\.accountLink='manage-users'/);assert.match(script.body,/function loadManageUsers\(\)/);assert.match(script.body,/\/invitations/);assert.match(script.body,/invitationTokenFromUrl/);});

test('Settings exposes authenticator MFA and firm-admin security response controls',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/id="loginMfaCode"/);const script=await raw(handler,'/app.js');assert.match(script.body,/Account protection/);assert.match(script.body,/Set up authenticator MFA/);assert.match(script.body,/Require MFA for this firm/);assert.match(script.body,/Remove firm access/);assert.match(script.body,/\/v1\/auth\/mfa\/enroll/);assert.match(script.body,/\/v1\/auth\/mfa\/confirm/);assert.match(script.body,/security\/events/);assert.match(script.body,/security\/sessions/);assert.match(script.body,/security\/policy/);assert.match(script.body,/memberships\/\$\{encodeURIComponent\(member\.userId\)\}/);assert.match(script.body,/FIRM_MFA_REQUIRED.*showView\('settings'\)/s);assert.match(script.body,/Complete account protection before opening firm work/);assert.match(script.body,/security\/sign-out-all/);assert.match(script.body,/SIGN OUT ALL/);});
...
test('Workspace communication activity requires an explicit case or no-case choice and stores both scopes canonically',async()=>{const handler=fixture();const [page,script]=await Promise.all([raw(handler,'/'),raw(handler,'/app.js')]);assert.match(page.body,/workspace-communication-activity/);assert.match(page.body,/connected-mail-shell/);assert.match(script.body,/No case — store as firm event/);assert.match(script.body,/Choose a case or select “No case — store as firm event/);assert.match(script.body,/matter=requestedMatter\(prompt\)/);assert.doesNotMatch(script.body,/matters\.length===1\?matters\[0\]/);assert.match(script.body,/firm-communications/);assert.match(script.body,/link-case/);assert.match(script.body,/Open draft in/);assert.match(script.body,/outlook\.office\.com\/mail\/deeplink\/compose/);assert.match(script.body,/renderConnectedCommunicationWorkspace/);assert.match(script.body,/Open related case/);assert.match(script.body,/It can be connected to a case later/);});

test('connected calendars share the mailbox connection and render canonical review-gated events',async()=>{const handler=fixture();const page=await raw(handler,'/');const script=await raw(handler,'/app.js');assert.match(page.body,/Google Workspace and Microsoft 365 can add an Atlas-proposed event only after an attorney formally approves it in While You Were Gone/);assert.match(script.body,/calendar:\{title:'Calendar'.*type:'calendar_event'.*readOnly:true/);assert.match(script.body,/\['calendar_event','deadline'\]\.includes\(object\.type\)/);assert.match(script.body,/item\.state\?\.startsAt\?\?item\.state\?\.start/);assert.match(script.body,/Approve calendar event/);assert.match(script.body,/calendarDelivery\?\.status==='published'/);assert.match(script.body,/Sync email & calendar/);assert.match(script.body,/approved event\(s\) added to/);assert.match(script.body,/Open in \$\{emailProviderLabel/);assert.match(script.body,/function safeExternalUrl\(value\)/);assert.match(script.body,/window\.open\(authorizationUrl\.href,'atlas-mailbox-authorization','popup,width=720,height=820'\)/);assert.match(script.body,/popup\.opener=null/);});
...
test('OAuth callback renders a safe failure page when Microsoft consent is denied',async()=>{let completion;const cms={async completeAuthorization(input){completion=input;throw new AtlasError('CMS_AUTHORIZATION_DENIED','denied',400,{provider:'microsoft',error:'access_denied'});}};const handler=createAtlasHandler(new AtlasService(new InMemoryRepository()),{config:{maxBodyBytes:1_048_576,corsOrigins:[]},ready:async()=>true,cms});const callback=await raw(handler,'/v1/cms/oauth/callback?state=state&error=access_denied&error_description=%3Cscript%3Ebad%3C%2Fscript%3E');assert.equal(callback.status,400);assert.equal(completion.error,'access_denied');assert.match(callback.headers['content-type'],/text\/html/);assert.match(callback.body,/Connection canceled/);assert.doesNotMatch(callback.body,/script|error_description|access_denied/i);});

test('Tasks follows Calendar and shares the homepage queue of tasks deadlines and Atlas suggestions',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/data-view="calendar">Calendar<\/button><button data-view="tasks">Tasks<\/button><button data-view="legal-research">Legal Research/);const script=await raw(handler,'/app.js');assert.match(script.body,/tasks:\{title:'Tasks'/);assert.match(script.body,/if\(name==='tasks'\)\{const items=homeWorkItems\(\)/);assert.match(script.body,/items\.forEach\(item=>appendWorkQueueItem\(list,item\)\)/);assert.match(script.body,/open canonical task, case deadline, or pending Atlas task suggestion/);});

test('Legal Research follows Tasks and provides one live source-grounded Atlas conversation',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/data-view="tasks">Tasks<\/button><button data-view="legal-research">Legal Research<\/button><button data-view="contacts">Contacts/);assert.match(page.body,/id="view-legal-research"/);assert.match(page.body,/What do you need researched?/);assert.match(page.body,/Best available live source/);assert.match(page.body,/Cited public web/);assert.match(page.body,/Westlaw/);assert.match(page.body,/LexisNexis/);assert.match(page.body,/Atlas never asks for or stores an attorney’s Westlaw or Lexis password/);const script=await raw(handler,'/app.js');assert.match(script.body,/legal-research\/providers/);assert.match(script.body,/legal-research\/capabilities/);assert.match(script.body,/legal-research\/chat/);assert.match(script.body,/legal-research\/search/);assert.match(script.body,/function submitLegalResearchChat\(\)/);assert.match(script.body,/legal_research_analysis/);assert.match(script.body,/research-citations/);assert.match(script.body,/Licensed legal sources/);assert.match(script.body,/submitChatOnEnter\('legalResearchChatInput','legalResearchChatForm'\)/);});

test('licensed legal research HTTP routes preserve read and write authorization boundaries',async()=>{const calls=[];const legalResearch={listProviders(){return[{name:'westlaw',configured:true}];},async search(workspaceId,input,userId){calls.push({workspaceId,input,userId});return {research:{id:'obj_research',type:'legal_research'},citations:[]};}};const permissions=[];const identity={async authenticate(){return{id:'usr_research'};},async authorize(workspaceId,userId,permission){permissions.push([workspaceId,userId,permission]);}};const handler=createAtlasHandler(new AtlasService(new InMemoryRepository()),{config:{maxBodyBytes:1_048_576,corsOrigins:[]},ready:async()=>true,identity,legalResearch});const headers={authorization:'Bearer test'};const providers=await json(handler,'/v1/workspaces/wsp_research/legal-research/providers',{headers});assert.equal(providers.status,200);const searched=await json(handler,'/v1/workspaces/wsp_research/legal-research/search',{method:'POST',headers,body:JSON.stringify({provider:'westlaw',query:'controlling discovery standard'})});assert.equal(searched.status,200);assert.equal(calls[0].userId,'usr_research');assert.deepEqual(permissions.map(item=>item[2]),['workspace:read','workspace:write']);});

test('live conversational legal research exposes capabilities and writes only through an authenticated firm boundary',async()=>{const calls=[];const assistant={legalResearchCapabilities(){return{aiConfigured:true,liveAvailable:true,publicWeb:{configured:true},licensed:[]};},async research(input){calls.push(input);return{answer:'Cited answer',conversationId:'aic_research',webSources:[{title:'Court',url:'https://court.example'}],sources:[],actionProposals:[]};}};const permissions=[];const identity={async authenticate(){return{id:'usr_research'};},async authorize(workspaceId,userId,permission){permissions.push([workspaceId,userId,permission]);}};const handler=createAtlasHandler(new AtlasService(new InMemoryRepository()),{config:{maxBodyBytes:1_048_576,corsOrigins:[]},ready:async()=>true,identity,assistant,legalResearch:{listProviders(){return[];}}});const headers={authorization:'Bearer test'};const capabilities=await json(handler,'/v1/workspaces/wsp_research/legal-research/capabilities',{headers});assert.equal(capabilities.status,200);assert.equal(capabilities.body.data.publicWeb.configured,true);const chat=await json(handler,'/v1/workspaces/wsp_research/legal-research/chat',{method:'POST',headers,body:JSON.stringify({prompt:'Research current law',sourceMode:'public_web',matterId:'obj_case'})});assert.equal(chat.status,200);assert.equal(calls[0].userId,'usr_research');assert.equal(calls[0].workspaceId,'wsp_research');assert.equal(calls[0].matterId,'obj_case');assert.deepEqual(permissions.map(item=>item[2]),['workspace:read','workspace:write']);});
...
test('firm onboarding atomically creates the owner subscription and authenticated workspace',async()=>{
  ...
  const profile=await json(handler,'/v1/me',{headers});assert.equal(profile.status,200);assert.deepEqual({name:profile.body.data.name,email:profile.body.data.email},{name:'First Owner',email:'owner@newfirm.test'});assert.equal('passwordHash' in profile.body.data,false);
...
test('firm administrators manage MFA policy and reversible member access over HTTP',async()=>{
  ...
  const handler=createAtlasHandler(service,{identity,config:{maxBodyBytes:1_048_576,corsOrigins:[]},ready:async()=>true});
  const owner=(await json(handler,'/v1/auth/register-firm',{method:'POST',body:JSON.stringify({firmName:'Access Control Firm',name:'Owner',email:'owner@access-http.test',password:'correct horse battery staple'})})).body.data;
  const member=await identity.register({name:'Paralegal',email:'paralegal@access-http.test',password:'correct horse battery staple'});
  await identity.addMembership(owner.workspace.id,member.user.id,'paralegal');
  const ownerHeaders={authorization:`Bearer ${owner.accessToken}`};
  const memberHeaders={authorization:`Bearer ${member.accessToken}`};
  const policy=await json(handler,`/v1/workspaces/${owner.workspace.id}/security/policy`,{headers:ownerHeaders});
  assert.equal(policy.body.data.requireMfa,false);
  const saved=await json(handler,`/v1/workspaces/${owner.workspace.id}/security/policy`,{method:'PATCH',headers:ownerHeaders,body:JSON.stringify({requireMfa:false})});
  assert.equal(saved.status,200);
  const deactivated=await json(handler,`/v1/workspaces/${owner.workspace.id}/memberships/${member.user.id}/deactivate`,{method:'POST',headers:ownerHeaders,body:JSON.stringify({reason:'Offboarded'})});
  assert.equal(deactivated.body.data.active,false);
  assert.equal(deactivated.body.data.sessionsRevoked,true);
  const denied=await json(handler,`/v1/workspaces/${owner.workspace.id}`,{headers:memberHeaders});
  assert.equal(denied.status,401);
  assert.equal(denied.body.error.code,'ACCESS_TOKEN_REVOKED');
  const restored=await json(handler,`/v1/workspaces/${owner.workspace.id}/memberships/${member.user.id}/reactivate`,{method:'POST',headers:ownerHeaders,body:'{}'});
  assert.equal(restored.body.data.active,true);
  const relogged=await identity.login({email:'paralegal@access-http.test',password:'correct horse battery staple'});
  assert.equal((await json(handler,`/v1/workspaces/${owner.workspace.id}`,{headers:{authorization:`Bearer ${relogged.accessToken}`}})).status,200);
});
...
test('case assistant conversations are server-bound to one authorized canonical matter',async()=>{
  const repository=new InMemoryRepository();const service=new AtlasService(repository);const identity=new IdentityService(repository,new TokenService('a'.repeat(32)));let captured=null;
  const assistant={async query(input){captured=input;return{answer:'Case context loaded.',conversationId:'aic_case',sources:[],actionProposals:[]};}};
  const handler=createAtlasHandler(service,{config:{maxBodyBytes:1_048_576,corsOrigins:[]},ready:async()=>true,identity,assistant});
  const registered=(await json(handler,'/v1/auth/register',{method:'POST',body:JSON.stringify({email:'case-chat@example.com',name:'Case Chat',password:'correct password long enough'})})).body.data;const headers={authorization:`Bearer ${registered.accessToken}`};const workspace=(await json(handler,'/v1/workspaces',{method:'POST',headers,body:JSON.stringify({name:'Case Chat Firm'})})).body.data;
  const matter=await service.createObject(workspace.id,{dimension:'matter',type:'civil',title:'Reed v. Northline'});const task=await service.createObject(workspace.id,{parentObjectId:matter.id,dimension:'operation',type:'task',title:'Review discovery'});
  const response=await json(handler,`/v1/workspaces/${workspace.id}/assistant/query`,{method:'POST',headers,body:JSON.stringify({prompt:'What should I do next?',contextObjectId:matter.id})});assert.equal(response.status,200);assert.equal(captured.prompt,'What should I do next?');assert.match(captured.developerContext,new RegExp(matter.id));assert.match(captured.developerContext,/call get_canonical_context/);assert.match(captured.developerContext,/Consequential actions remain proposals requiring human review/);
  const invalid=await json(handler,`/v1/workspaces/${workspace.id}/assistant/query`,{method:'POST',headers,body:JSON.stringify({prompt:'Help',contextObjectId:task.id})});assert.equal(invalid.status,400);assert.equal(invalid.body.error.code,'AI_CONTEXT_INVALID');
  const otherWorkspace=await service.createWorkspace({name:'Other Firm'});const secret=await service.createObject(otherWorkspace.id,{dimension:'matter',type:'civil',title:'Secret Case'});const crossFirm=await json(handler,`/v1/workspaces/${workspace.id}/assistant/query`,{method:'POST',headers,body:JSON.stringify({prompt:'Help',contextObjectId:secret.id})});assert.equal(crossFirm.status,404);
});

test('case-task execution validates case ownership and prepares substantive draft work without closing the task',async()=>{
  const repository=new InMemoryRepository();const service=new AtlasService(repository);const identity=new IdentityService(repository,new TokenService('a'.repeat(32)));let turn=0;
  const model={async complete(input){turn+=1;if(turn===1){const prompt=input.messages.at(-1).content;assert.match(prompt,/Perform the existing Atlas case task now/);assert.match(prompt,/Generate discovery plan/);assert.match(prompt,/get_canonical_context/);assert.match(prompt,/Do not create another task and do not mark this task complete/);return {toolCalls:[{id:'canonical_context',name:'get_canonical_context',arguments:{objectId:task.id}}]};}if(turn===2)return {toolCalls:[{id:'draft_plan',name:'propose_create_document',arguments:{title:'Discovery Plan',documentType:'discovery_plan',matterId:matter.id,content:'Draft discovery objectives, written discovery sequence, deposition sequencing, and attorney-review checkpoints.'}}]};return {text:'I prepared a source-grounded discovery plan for attorney review. The original task remains open.'};}};
  const assistant=new AtlasAssistant(model,new AtlasToolRegistry(service),{repository});const handler=createAtlasHandler(service,{config:{maxBodyBytes:1_048_576,corsOrigins:[]},ready:async()=>true,identity,assistant});
  const registered=(await json(handler,'/v1/auth/register',{method:'POST',body:JSON.stringify({email:'task-work@example.com',name:'Task Attorney',password:'correct password long enough'})})).body.data;const headers={authorization:`Bearer ${registered.accessToken}`};const workspace=(await json(handler,'/v1/workspaces',{method:'POST',headers,body:JSON.stringify({name:'Task Work Firm'})})).body.data;
  const matter=await service.createObject(workspace.id,{dimension:'matter',type:'civil',title:'Morgan v. Lakeside',state:{caseNumber:'2026-CV-44',courtName:'Superior Court'}});const task=await service.createObject(workspace.id,{parentObjectId:matter.id,dimension:'operation',type:'task',title:'Generate discovery plan',state:{status:'open'}});const otherMatter=await service.createObject(workspace.id,{dimension:'matter',type:'civil',title:'Other Case'});const canonicalContext=await json(handler,`/v1/workspaces/${workspace.id}/objects/${task.id}/context`,{headers});assert.equal(canonicalContext.status,200);assert.equal(canonicalContext.body.data.matter.id,matter.id);assert.deepEqual(new Set(canonicalContext.body.data.objects.map(item=>item.id)),new Set([matter.id,task.id]));
  const mismatch=await json(handler,`/v1/workspaces/${workspace.id}/matters/${otherMatter.id}/tasks/${task.id}/perform`,{method:'POST',headers});assert.equal(mismatch.status,409);assert.equal(mismatch.body.error.code,'CASE_TASK_MISMATCH');assert.equal(turn,0);
  const performed=await json(handler,`/v1/workspaces/${workspace.id}/matters/${matter.id}/tasks/${task.id}/perform`,{method:'POST',headers});assert.equal(performed.status,200);assert.equal(performed.body.data.task.id,task.id);assert.equal(performed.body.data.matter.id,matter.id);assert.match(performed.body.data.answer,/discovery plan/);assert.equal(performed.body.data.actionProposals[0].actionType,'create_document');assert.equal(performed.body.data.actionProposals[0].input.matterId,matter.id);assert.equal((await service.getObject(workspace.id,task.id)).state.status,'open');
  const proposal=performed.body.data.actionProposals[0];const approved=await json(handler,`/v1/workspaces/${workspace.id}/assistant/actions/${proposal.id}/decision`,{method:'POST',headers,body:JSON.stringify({version:proposal.version,decision:'approve'})});assert.equal(approved.body.data.result.parentObjectId,matter.id);assert.equal(approved.body.data.result.type,'discovery_plan');assert.equal(approved.body.data.result.state.status,'draft');assert.equal(approved.body.data.result.state.filed,false);assert.equal((await service.getObject(workspace.id,task.id)).state.status,'open');
});
