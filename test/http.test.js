...
test('frontend uses a versioned deferred script that executes in the local pilot browser',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/<span class="sr-only">Reply to Atlas or ask anything<\/span><textarea/);assert.match(page.body,/placeholder="Reply to Atlas or ask anything…"/);assert.match(page.body,/<script defer src="\.\/app\.js\?v=0\.53\.0-file-security-1"><\/script>/);});
...
test('health endpoint reports the running release', async () => {
  ...
  assert.deepEqual(response.body, { data: { status: 'ok', version: '0.53.0' } });
