...
test('frontend uses a versioned deferred script that executes in the local pilot browser',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/<span class="sr-only">Reply to Atlas or ask anything<\/span><textarea/);assert.match(page.body,/placeholder="Reply to Atlas or ask anything…"/);assert.match(page.body,/<script defer src="\.\/app\.js\?v=0\.47\.0-document-intelligence-1"><\/script>/);});
...
test('mail connector discovery is authenticated and OAuth callback exposes no credentials',async()=>{const cms={listProviders(){return [{name:'google',capabilities:{readOnly:true,resources:['email','calendar']}}];},async completeAuthorization(){return {id:'cms_mail',provider:'google',status:'connected',accessMode:'read_only'};}};const identity={async authenticate(){return {id:'usr_mail'};},async authorize(){}};const handler=createAtlasHandler(new AtlasService(new InMemoryRepository()),{config:{maxBodyBytes:1_048_576,corsOrigins:[]},ready:async()=>true,identity,cms});const providers=await json(handler,'/v1/workspaces/wsp_mail/cms/providers',{headers:{authorization:'Bearer test'}});assert.equal(providers.status,200);assert.equal(providers.body.data[0].name,'google');const callback=await raw(handler,'/v1/cms/oauth/callback?state=state&code=code');assert.equal(callback.status,200);assert.match(callback.headers['content-type'],/text\/html/);assert.match(callback.body,/Connection complete/);assert.doesNotMatch(callback.body,/access_token|refresh_token|credentialRef/);});
...
test('health endpoint reports the running release', async () => {
  ...
  assert.deepEqual(response.body, { data: { status: 'ok', version: '0.47.0' } });
