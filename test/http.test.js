...
test('the twin dock is excluded from Home and Workspace but inherited by other authenticated views',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/id="twinDock"/);assert.match(page.body,/id="twinForm"/);assert.match(page.body,/Ask Atlas from anywhere/);assert.match(page.body,/aria-label="Atlas digital twin"/);const script=await raw(handler,'/app.js');assert.match(script.body,/submitTwinCommand/);assert.match(script.body,/conversationId/);assert.match(script.body,/assistant\/query/);assert.match(script.body,/actionProposals/);assert.match(script.body,/twinToggle/);assert.match(script.body,/twinDockExcludedViews=new Set\(\['dashboard','workspace'\]\)/);assert.match(script.body,/function shouldShowTwinDock\(viewName\)\{return !twinDockExcludedViews\.has\(viewName\);\}/);assert.match(script.body,/!shouldShowTwinDock\(name\)/);});
