...
test('frontend uses a versioned deferred script that executes in the local pilot browser',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/<span class="sr-only">Reply to Atlas or ask anything<\/span><textarea/);assert.match(page.body,/placeholder="Reply to Atlas or ask anything…"/);assert.match(page.body,/<script defer src="\.\/app\.js\?v=1\.0\.0-rc\.1-account-profiles-1"><\/script>/);});
...
test('Accounting exposes a read-only QuickBooks connection and canonical financial mirror',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/account-profiles-1/);const script=await raw(handler,'/app.js');assert.match(script.body,/QuickBooks Online/);assert.match(script.body,/Intuit’s secure consent screen/);assert.match(script.body,/cms\/quickbooks\/authorize/);assert.match(script.body,/quickBooksSync/);assert.match(script.body,/QuickBooks finances are mirrored read-only/);assert.match(script.body,/externalSource/);});
...
test('Attorney inbox matches the native firm twin height on the desktop homepage',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/#view-dashboard>\.command-center-grid\{align-items:stretch\}/);assert.match(page.body,/#view-dashboard>\.command-center-grid>\.command-console\{grid-column:1;grid-row:1;height:100%\}/);assert.match(page.body,/#view-dashboard>\.command-center-grid>\.command-rail>#reviewInbox\{grid-column:2;grid-row:1;display:flex;min-height:0;height:100%;flex-direction:column\}/);assert.match(page.body,/#view-dashboard>\.command-center-grid>\.command-rail>#reviewInbox \.feed\{flex:1;min-height:0;max-height:390px;overflow-y:auto;scrollbar-gutter:stable\}/);assert.match(page.body,/@media\(max-width:1050px\)/);});
test('every search bar offers linked familiar Atlas destinations',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/app\.js\?v=1\.0\.0-rc\.1-account-profiles-1/);const script=await raw(handler,'/app.js');assert.match(script.body,/function installAtlasSearchAutocomplete\(\)/);assert.match(script.body,/input\[type="search"\]/);assert.match(script.body,/Familiar Atlas destinations/);assert.match(script.body,/function matchingAtlasDestinations\(query\)/);assert.match(script.body,/function openSearchObject\(object\)/);assert.match(script.body,/destination\.activate\(\)/);assert.match(script.body,/new MutationObserver/);assert.match(script.body,/aria-autocomplete/);assert.match(script.body,/ArrowDown/);assert.match(script.body,/accountingPageLabels/);});
