...
test('serves the connected phase-one client from the application origin',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.equal(page.status,200);assert.match(page.headers['content-type'],/text\/html/);assert.match(page.body,/While You Were Gone/);assert.match(page.body,/continuously aware digital twin/);assert.match(page.body,/commandForm/);assert.match(page.body,/app-shell/);assert.match(page.body,/data-view="matters"/);assert.match(page.body,/matterCount/);assert.match(page.body,/matterList/);assert.match(page.body,/onboardingForm/);assert.match(page.body,/matterForm/);assert.match(page.body,/data-matter-tab="timeline"/);const script=await raw(handler,'/app.js');assert.match(script.headers['content-type'],/javascript/);assert.match(script.body,/authorization:`Bearer/);assert.match(script.body,/assistant\/query/);assert.match(script.body,/conversationId/);assert.match(script.body,/actionProposals/);assert.match(script.body,/assistant\/actions/);assert.match(script.body,/Approve draft/);assert.match(script.body,/intelligence\/observations/);assert.match(script.body,/Accept/);assert.match(script.body,/loadPilotData/);assert.match(script.body,/register-firm/);assert.match(script.body,/openMatter/);assert.match(script.body,/matters\/\$\{encodeURIComponent\(matter\.id\)\}\/health/);assert.match(script.body,/events\?parentObjectId/);});
