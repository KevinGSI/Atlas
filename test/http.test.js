...
test('frontend uses a versioned deferred script that executes in the local pilot browser',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/placeholder="Ask or request anything"/);assert.match(page.body,/<script defer src="\.\/app\.js\?v=0\.36\.0-15"><\/script>/);});
