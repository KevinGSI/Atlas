...
test('frontend uses a versioned deferred script that executes in the local pilot browser',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/<span class="sr-only">Ask or request anything<\/span><textarea/);assert.match(page.body,/placeholder="Ask or request anything"/);assert.doesNotMatch(page.body,/>Tell the firm twin what you need</);assert.match(page.body,/<script defer src="\.\/app\.js\?v=0\.36\.0-17"><\/script>/);});

test('Home uses the coded AI Command Center application template',async()=>{const page=await raw(fixture(),'/');assert.match(page.body,/class="view command-center"/);assert.match(page.body,/AI Command Center/);assert.match(page.body,/Native firm twin active/);assert.match(page.body,/class="card command command-console"/);assert.match(page.body,/Command Atlas/);assert.match(page.body,/Operational picture/);assert.match(page.body,/Connected to canonical firm work/);assert.match(page.body,/class="card review-center"/);assert.match(page.body,/Attorney review/);assert.match(page.body,/\.nav button\[data-view="workspace"\]:before\{content:'✦'\}/);assert.doesNotMatch(page.body,/Recent Activity/);});

test('homepage summary controls are recognizable buttons with relevant working destinations',async()=>{const handler=fixture();const page=await raw(handler,'/');for(const id of ['openCasesMetric','openTasksMetric','reviewDeadlinesMetric','openReviewMetric'])assert.match(page.body,new RegExp(`id="${id}"`));assert.match(page.body,/aria-label="Open all cases"/);assert.match(page.body,/View task workspace/);assert.match(page.body,/Ask Atlas to review/);assert.match(page.body,/Open review inbox/);assert.match(page.body,/id="reviewInbox"/);const script=await raw(handler,'/app.js');assert.match(script.body,/openCasesMetric.*showView\('matters'\)/);assert.match(script.body,/openTasksMetric.*showView\('tasks'\)/);assert.match(script.body,/reviewDeadlinesMetric.*routeHomeCommand\('Review every approaching or missed deadline/);assert.match(script.body,/openReviewMetric.*reviewInbox.*scrollIntoView/);});

test('Workspace unifies filterable lawyer work and the same native firm twin',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/data-view="dashboard">Home<\/button><button data-view="workspace">Workspace<\/button><button data-view="matters">Cases/);assert.match(page.body,/id="view-workspace"/);assert.match(page.body,/id="workspaceAssistantForm"/);assert.match(page.body,/id="workspaceAssistantTranscript"/);assert.match(page.body,/id="workspaceSearch"/);assert.match(page.body,/id="workspaceFilter"/);assert.match(page.body,/AI drafts/);assert.match(page.body,/Needs review/);assert.match(page.body,/Everything in progress/);const script=await raw(handler,'/app.js');assert.match(script.body,/function submitWorkspaceCommand\(prompt\)/);assert.match(script.body,/workspaceAssistant.*assistant\/query/s);assert.match(script.body,/function workspaceEntries\(\)/);assert.match(script.body,/workspaceActions/);assert.match(script.body,/workspaceAwareness/);assert.match(script.body,/assistant\/actions/);assert.match(script.body,/function renderWorkspaceWork\(\)/);assert.match(script.body,/workspaceSearch.*renderWorkspaceWork/);assert.match(script.body,/name==='workspace'\?'Workspace'/);});

test('Home routes navigation directly and moves work requests into Workspace for execution',async()=>{const script=await raw(fixture(),'/app.js');assert.match(script.body,/function routeHomeCommand\(prompt\)/);assert.match(script.body,/requestedMatter\(text\)/);assert.match(script.body,/openMatter\(matter\)/);assert.match(script.body,/showView\('workspace'\);submitWorkspaceCommand\(text\)/);assert.match(script.body,/commandForm.*routeHomeCommand/s);assert.match(script.body,/data-prompt.*routeHomeCommand/s);assert.match(script.body,/case\|cases\|matter\|matters/);assert.match(script.body,/most important \(task\|tasks\|work\)/);});
...
test('all present and future authenticated views inherit the twin dock while Home is the sole explicit exception',async()=>{const handler=fixture();const page=await raw(handler,'/');assert.match(page.body,/id="twinDock"/);assert.match(page.body,/id="twinForm"/);assert.match(page.body,/Ask Atlas from anywhere/);assert.match(page.body,/aria-label="Atlas digital twin"/);const script=await raw(handler,'/app.js');assert.match(script.body,/submitTwinCommand/);assert.match(script.body,/conversationId/);assert.match(script.body,/assistant\/query/);assert.match(script.body,/actionProposals/);assert.match(script.body,/twinToggle/);assert.match(script.body,/twinDockExcludedViews=new Set\(\['dashboard'\]\)/);assert.match(script.body,/function shouldShowTwinDock\(viewName\)\{return !twinDockExcludedViews\.has\(viewName\);\}/);assert.match(script.body,/!shouldShowTwinDock\(name\)/);});
