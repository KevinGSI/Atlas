...
test('an accepted email-to-case match files its canonical attachments into that case',async()=>{
  const {repository,service,workspace}=await fixture();
  const matter=await service.createObject(workspace.id,{dimension:'matter',type:'civil',title:'Reed v. Northline'});
  const email=await service.createObject(workspace.id,{dimension:'operation',type:'incoming_email',title:'Discovery filing',state:{from:'court@example.com'}});
  const document=await service.createObject(workspace.id,{dimension:'document',type:'incoming_attachment',title:'filing.pdf',state:{provenance:{kind:'email_attachment',sourceObjectId:email.id,connector:'microsoft'}}});
  await service.createRelationship(workspace.id,{fromObjectId:email.id,toObjectId:document.id,type:'has_attachment'});
  const observation=await repository.createIntelligenceObservation({id:'ino_email_matter',workspaceId:workspace.id,jobId:'inj_email_matter',sourceObjectId:email.id,kind:'matter_match',data:{matterId:matter.id},confidence:.93,sourceLocation:{subject:true},provider:'test',status:'candidate',reviewedBy:null,reviewedAt:null,createdAt:'2026-07-10T12:00:00.000Z'});
  const accepted=await service.decideIntelligenceObservation(workspace.id,observation.id,{decision:'accept'},'usr_reviewer');
  assert.equal(accepted.observation.status,'accepted');
  assert.equal((await repository.getObject(workspace.id,email.id)).state.matterId,matter.id);
  const filed=await repository.getObject(workspace.id,document.id);
  assert.equal(filed.state.matterId,matter.id);
  assert.equal(filed.state.matterAssociation.source,'accepted_intelligence_match');
  assert.equal(filed.state.provenance.sourceObjectId,email.id);
  const relationships=await repository.listRelationships(workspace.id);
  assert.equal(relationships.some(item=>item.fromObjectId===email.id&&item.toObjectId===matter.id&&item.type==='intelligence_matched_to'),true);
  assert.equal(relationships.some(item=>item.fromObjectId===email.id&&item.toObjectId===document.id&&item.type==='has_attachment'),true);
});
...
test('stored document bytes route to file intelligence before a preferred wildcard model',async()=>{const content=Buffer.from('court notice');const sha256=(await import('node:crypto')).createHash('sha256').update(content).digest('hex');let analyzed;const documentProvider=new DocumentIntelligenceProvider({async analyzeFile(input){analyzed=input;return {documentAnalysis:{documentType:'notice',summary:'A court notice states that a response is due.',confidence:.91,suggestedTitle:'Court Notice',caseNumber:null,court:null,documentDate:null,parties:[],attorneys:[],organizations:[],keyDates:[],requiresAttorneyReview:true},observations:[{kind:'deadline',data:{title:'Response due'},confidence:.9}],actionProposals:[],awareness:{category:'document_upload',priority:'normal',headline:'Notice analyzed',summary:'A court notice is ready for review.'},retrievalChunks:[]};}},{async read(){return content;}});const wildcard={capabilities(){return {triggers:['*']};},async analyze(){throw new Error('wildcard should not own document bytes');}};const registry=new IntelligenceProviderRegistry().register('document-analysis',documentProvider).register('configured-model',wildcard);assert.equal(registry.resolveFor('attachment.received','configured-model').name,'document-analysis');const result=await documentProvider.analyze({event:{document:{id:'obj_doc',title:'notice.pdf',state:{storageRef:'atlas-blob://wsp/sha',sha256,size:content.length,mediaType:'application/pdf'}}},context:{workspaceId:'wsp_1'}});assert.equal(analyzed.filename,'notice.pdf');assert.match(analyzed.instruction,/documentAnalysis is required/);assert.equal(result.documentAnalysis.documentType,'notice');assert.equal(result.observations[0].kind,'deadline');});
test('document analysis is cataloged on the canonical document with a visible summary and legal type',async()=>{const {repository,service,workspace}=await fixture();const document=await service.createObject(workspace.id,{dimension:'document',type:'uploaded_document',title:'filing.pdf',state:{storageRef:'atlas-blob://test/file',sha256:'abc',size:100,mediaType:'application/pdf',extractionStatus:'pending'}});const projector=new IntelligenceProjectionService(()=> '2026-07-10T12:01:00.000Z');const result={documentAnalysis:{documentType:'motion',summary:'The defendant moves to compel complete interrogatory responses and requests fees.',confidence:.93,suggestedTitle:'Motion to Compel',caseNumber:'2026-CV-44',court:'Superior Court',parties:['Taylor Morgan','Lakeside Property Group'],keyDates:[{label:'Response due',date:'2026-07-24',sourceLocation:{page:7}}],requiresAttorneyReview:true},observations:[{kind:'classification',data:{documentType:'motion'},confidence:.93,sourceLocation:{page:1}}],actionProposals:[],awareness:{category:'document_upload',priority:'normal',headline:'Motion cataloged',summary:'A motion to compel was summarized for review.'}};const projected=await repository.transaction(()=>projector.project(repository,{id:'inj_document_catalog',workspaceId:workspace.id,objectId:document.id,triggerType:'attachment.received'},'openai',result));assert.equal(projected.catalogedDocument.state.documentType,'motion');assert.equal(projected.catalogedDocument.state.summary,result.documentAnalysis.summary);assert.equal(projected.catalogedDocument.state.extractionStatus,'completed');assert.equal(projected.catalogedDocument.state.documentAnalysis.caseNumber,'2026-CV-44');assert.equal(projected.catalogedDocument.state.documentAnalysis.status,'cataloged');assert.equal((await repository.listEvents(workspace.id,document.id)).some(event=>event.type==='document.analyzed'),true);assert.equal((await repository.listAudits(workspace.id,document.id)).some(audit=>audit.action==='document.analyzed'),true);});
