...
const valid = {
  ...
  CMS_SYNC_ENABLED:'true', PUBLIC_BASE_URL:'https://atlas.example.test', DOCUMENT_STORAGE_PATH:'/var/data/atlas-documents'
};
...
test('launch readiness refuses ephemeral document storage',()=>{const result=evaluateLaunchReadiness({...valid,DOCUMENT_STORAGE_PATH:''});assert.equal(result.ready,false);assert.ok(result.missing.includes('DOCUMENT_STORAGE_PATH'));});
