...
test('live PostgreSQL applies every migration and persists an isolated Atlas workflow',{skip:!databaseUrl},async()=>{
  ...
  try{
    ...
    const migrations=join(dirname(fileURLToPath(import.meta.url)),'..','db','migrations');const applied=await runMigrations(executor,migrations);assert.equal(applied.length,17);assert.equal((await runMigrations(executor,migrations)).length,0);
    const tables=await client.query("SELECT count(*)::int count FROM information_schema.tables WHERE table_schema=$1",[schema]);assert.equal(tables.rows[0].count,25);
    const repository=new PostgresRepository(executor);const clock=()=> '2026-07-10T12:00:00.000Z';const service=new AtlasService(repository,clock);const workspace=await service.createWorkspace({name:'Live PostgreSQL Firm'});const matter=await service.createObject(workspace.id,{dimension:'matter',type:'civil',title:'Live Database Matter'});assert.equal((await service.getObject(workspace.id,matter.id)).title,'Live Database Matter');
    ...
    const event=await service.createEvent(workspace.id,{parentObjectId:matter.id,type:'live.test',actorId:'usr_live',source:'integration'});await assert.rejects(()=>client.query('UPDATE atlas_timeline_event SET source=$1 WHERE id=$2',['tampered',event.id]));
