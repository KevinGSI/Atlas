...
test('live PostgreSQL applies every migration and persists an isolated Atlas workflow',{skip:!databaseUrl},async()=>{
  ...
  try{
    ...
    const migrations=join(dirname(fileURLToPath(import.meta.url)),'..','db','migrations');const applied=await runMigrations(executor,migrations);assert.equal(applied.length,16);assert.equal((await runMigrations(executor,migrations)).length,0);
    const tables=await client.query("SELECT count(*)::int count FROM information_schema.tables WHERE table_schema=$1",[schema]);assert.equal(tables.rows[0].count,24);
