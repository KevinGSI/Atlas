...
test('live PostgreSQL applies every migration and persists an isolated Atlas workflow',{skip:!databaseUrl},async()=>{
  ...
  try{
    ...
    const migrations=join(dirname(fileURLToPath(import.meta.url)),'..','db','migrations');const expectedMigrations=await loadMigrations(migrations);const applied=await runMigrations(executor,migrations);assert.deepEqual(applied,expectedMigrations.map((item)=>item.name));assert.equal((await runMigrations(executor,migrations)).length,0);const history=await client.query('SELECT count(*)::int count FROM atlas_schema_migration');assert.equal(history.rows[0].count,expectedMigrations.length);
    const tables=await client.query("SELECT count(*)::int count FROM information_schema.tables WHERE table_schema=$1 AND table_name <> 'atlas_schema_migration'",[schema]);assert.equal(tables.rows[0].count,28);
    ...
    const ingestion=new AtlasIngestionService(repository,clock);await ingestion.ingestPhoneCall(workspace.id,{connector:'live-phone',externalId:'call-1',matterId:matter.id,direction:'incoming',durationSeconds:30},'usr_live');await ingestion.ingestDocument(workspace.id,{connector:'live-documents',externalId:'document-1',matterId:matter.id,filename:'notice.pdf',storageRef:'blob://notice',sha256:'abc123',mediaType:'application/pdf',size:128},'usr_live');const ingestionKinds=await client.query('SELECT kind FROM atlas_ingestion_record ORDER BY kind');assert.deepEqual(ingestionKinds.rows.map((row)=>row.kind),['document','phone_call']);
